...
```{r}
  ...
  cbind( names_params, df_parModes[,"CVPost_1"] )
...
```{r list_outF_C_SA}
  ...
  mat_outF_C_SA <- matrix( NA, nrow=npar, ncol=length(vars_Summary) )
  ...
  for(pi in 1:npar) {
    for(vi in 1:length(vars_Summary)) {
      ...
      mat_outF_C_SA[pi,vi] <- round( mean( S_pivi.f, na.rm=T ), 3 )
    }
  }
  ...
  Ssummary_C.vpmax <- sapply( 1:length(vars_Summary), function(vi){
    names( which.max( abs( mat_outF_C_SA[,vi] ) ) ) } )
  colnames( Ssummary_C.vpmax ) <- vars_Summary
  
  knitr::kable( cbind( t(Ssummary_C.v), Ssummary_C.vpmax ) )
```
...
```{r list_outF_G_SA}
  ...
  mat_outF_G_SA <- matrix( NA, nrow=npar, ncol=length(vars_Summary) )
  ...
  for(pi in 1:npar) {
    for(vi in 1:length(vars_Summary)) {
      ...
      mat_outF_G_SA[pi,vi] <- round( mean( S_pivi.f, na.rm=T ), 3 )
    }
  }
  ...
  Ssummary_G.vpmax <- sapply( 1:length(vars_Summary), function(vi){
    names( which.max( abs( mat_outF_G_SA[,vi] ) ) ) } )
  colnames( Ssummary_G.vpmax ) <- vars_Summary
  
  knitr::kable( cbind( t(Ssummary_G.v), Ssummary_G.vpmax ) )
```
