...
```{r Maps_CG, out.width="400px", fig.align="center", fig.cap="\\label{Maps_CG}Altitude maps for Costa Rica and Guatemala with locations of the coffee farms."}
  knitr::include_graphics("Maps_CG.png")
```
...
par( mfrow=c(1,3), mar=c(2,4,2,2), pty="s" )
  ...
  plot( y_T[,"Y.sim"], y_T[,"Y.obs"],
        ...
        xlab="Simulated coffee yield (kg ha-1)", ylab="Observed coffee yield (kg ha-1)",
...
```{r}
  plotimpacts_CG <- function( FC=oF1_C, FG=oF1_G, xvar="Nfert",
                              ...
                              title="Yield", legpos="topleft", xleg, npanel, ... ){
    ...
    plot  ( xC, yC,
      ...
      ylim=c( min(yC,yG,na.rm=T), max(yC,yG,na.rm=T)), col=costaCol, ... )
    ...
    legend( legpos,
            legend=c( paste("Costa Rica:",as.character(r2_C)),
                      paste("Guatemala:",as.character(r2_G)) ),
            col=c(costaCol,guateCol), lty=c(1), pch=c(16,15),pt.cex=c(0.8,0.7),cex=0.7, bg="transparent" ,bty="n")
  }
```

```{r FarmSimNfert, fig.height=4.5, fig.cap="\\label{fig:FarmSimNfert}Simulations of ecosystem services at farms in Costa Rica (black) and Guatemala (red). All plots are against N-fertilisation level (kg N ha-1 y-1)."}
  old.par <- par(no.readonly = TRUE)
  par( mfrow=c(2,4), mar=c(4,2,2,1), mgp=c(2,1,0), oma=c(4,2,2,2))

  yv <- "harvDM_f_ha1819" ; yv2 <- "harvDM_f_ha1920"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="Nfert", yvar=yv, yvar2=yv2,
                   legpos="bottomright", title="Coffee yield", xleg="N fertilisation (kg N ha-1)", npanel="")
  yv <- "harvDMST_3_hay"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="Nfert", yvar=yv,
                   legpos="topright"   , title="Timber" , xleg="N fertilisation (kg N ha-1)", npanel="")
  yv <- "harvCPT_f_hay"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="Nfert", yvar=yv,
                   legpos="topleft"    , title="Fruit" , xleg="N fertilisation (kg N ha-1)", npanel="")
  yv <- "D_Csoil"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="Nfert", yvar=yv,
                   legpos="bottomright",title="C-change" , xleg="N fertilisation (kg N ha-1)", npanel="")
  yv <- "Crunoff_fb"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="Nfert", yvar=yv,
                   legpos="topright"   , title="C-runoff" , xleg="N fertilisation (kg N ha-1)", npanel="")
  yv <- "NfixT_fb"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="Nfert", yvar=yv,
                   legpos="topright"    , title="N-fixation" , xleg="N fertilisation (kg N ha-1)", npanel="")
  yv <- "Nleaching_fb"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="Nfert", yvar=yv,
                   legpos="topleft"    , title="N-leaching" , xleg="N fertilisation (kg N ha-1)", npanel="")
  yv <- "Nemission_fb"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="Nfert", yvar=yv,
                   legpos="topleft"    , title="N-emission" , xleg="N fertilisation (kg N ha-1)", npanel="")
  #this is to create the legend below the plots
  par(fig = c(0, 1, 0, 1), oma = c(0, 0, 0, 0), mar = c(0, 0, 0, 0), new = TRUE)
  plot(0, 0, type = "n", bty = "n", xaxt = "n", yaxt = "n")
  legend("bottom",c("Costa Rica","Guatemala"),col=c(costaCol,guateCol),pch=c(16,15),lwd=2,pt.bg=c(costaCol,guateCol),cex=1,pt.cex=c(1.5,1.4),xpd=TRUE,horiz=TRUE,inset=0.03)
  par(old.par)  
...
```{r dES, fig.height=4, fig.align="center", out.width="80%", fig.cap="\\label{fig:dES}Changes in ecosystem services when fertilisation or shading is halved. 1:3 = yield of coffee, timber, fruit; 4 = soil carbon sequestration; 5 = erosive soil carbon loss in run-off; 6 = N-fixation; 7:8 = loss of N in leaching and emission. All changes are normalised to the country average."}
