...
Note various data corrections that were applied in the following code.

- Guatemala sample weights were about 12 times smaller than those in Costa Rica. We use a correction factor of 10 in the following code (assuming soils in Costa Rica are only 20% heavier than those in Guatemala, not 1100%). 
...
```{r}
  ...
  df_soil1.f  <- df_soil1.f0 %>%
    ...
    mutate( code = code + farm_number ) %>%
    arrange( code ) %>%
    rename( DM_AD.soil.sample    = "sub sample weight" ) %>%
    rename( DM_AD.soil.subsample_C = "airdry weight"   ) %>%
    rename( DM.soil.subsample_C    = "weight 105"      ) %>%
    rename( DM_AD.soil.subsample_G = "Gsubsample_105C" ) %>%
    rename( DM.soil.subsample_G    = "Gweight_105C"    ) %>%
    mutate( DM.soil.subsample_G    =
              ifelse( DM.soil.subsample_G < DM_AD.soil.subsample_G,
                      DM.soil.subsample_G,
                      NA ) ) %>% # Removing values where 105C increased weight
    mutate( DM.soil.sample =
              ifelse( country=="Costa Rica", DM_AD.soil.sample *
                        DM.soil.subsample_C / DM_AD.soil.subsample_C,
              ifelse( country=="Guatemala" , DM_AD.soil.sample * 10 *
                        DM.soil.subsample_G / DM_AD.soil.subsample_G,
                      NA )) ) %>% # Assuming Guatemala values are in dg.
    group_by( code, Profundity ) %>%
    summarise( DM.soil.layer = median(DM.soil.sample) * 
...
```{r, fig.height=8}
  ...
  plot  ( df_soil.f$altitude[iC], df_soil.f$DMfrac.soil[iC], main="DMfrac.soil",
          xlab="Alt. (m)", ylab="", ylim=c(0,ymax), pch=20 )
  ...
  plot  ( df_soil.f$altitude[iC], df_soil.f$CN.soil[iC], main="CN.soil",
          xlab="Alt. (m)", ylab="", ylim=c(0,ymax), pch=20 )
  ...
  plot  ( df_soil.f$altitude[iC], df_soil.f$C.soil[iC], main="C.soil",
          xlab="Alt. (m)", ylab="", ylim=c(0,ymax), pch=20 )
  ...
  plot  ( df_soil.f$altitude[iC], df_soil.f$N.soil[iC], main="N.soil",
          xlab="Alt. (m)", ylab="", ylim=c(0,ymax), pch=20 )
