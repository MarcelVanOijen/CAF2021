...
## Weather anomalies 2000-2018

```{r}
  prec.00_09 <- stack( list.files( dir_WC_prec.2000_2009, ".tif", full.names=T ) )
  tmax.00_09 <- stack( list.files( dir_WC_tmax.2000_2009, ".tif", full.names=T ) )
  tmin.00_09 <- stack( list.files( dir_WC_tmin.2000_2009, ".tif", full.names=T ) )

  prec_C.00_09 <- crop(prec.00_09, ext_C)
  prec_G.00_09 <- crop(prec.00_09, ext_G) ; rm(prec.00_09)
  tmax_C.00_09 <- crop(tmax.00_09, ext_C)
  tmax_G.00_09 <- crop(tmax.00_09, ext_G) ; rm(tmax.00_09)
  tmin_C.00_09 <- crop(tmin.00_09, ext_C)
  tmin_G.00_09 <- crop(tmin.00_09, ext_G) ; rm(tmin.00_09)

  # month <- c( "Jan", "Feb", "Mar", "Apr", "May", "Jun",
  #             "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" )
  # names(prec.A) <- names(srad.A) <- names(tmax.A) <-
  # names(tmin.A) <- names(vapr.A) <- names(wind.A) <- month
```
