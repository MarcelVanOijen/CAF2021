...
```{r}
  outputSummary <- function( output ) {
    ...
    fOut[,"NfixT_f"    ] <- mean( output[,iNfixT    ] ) * 1e4 * 365
    fOut[,"Nleaching_f"] <- mean( output[,iNleaching] ) * 1e4 * 365
    fOut[,"Crunoff_f"  ] <- mean( output[,iCrunoff  ] ) * 1e4 * 365
...
```{r}
  ...
  for(fi in 1:n_C.f) {
    ...
    for(i in 1:12) {
      ...
      outTypFr <- run_model( params, matrix_weather_fi, calendar_fert,
        calendar_prunC, calendar_prunT, calendar_thinT, NDAYS )
