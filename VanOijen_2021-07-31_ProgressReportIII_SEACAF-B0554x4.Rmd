...
```{r, out.width="100%"}
  ...
  yC1 <- fYieldsTM( parMatrix=parMAP, sfiles=sitesettings_filenames,
                    WC=TRUE, list_w=df_C.f$weather.WC, f=1 )
  par( pty="s" ) ; plotYieldsTM( yC1 )
  
  yG1 <- fYieldsTM( parMatrix=parMAP, sfiles=sitesettings_filenames,
                    WC=TRUE, list_w=df_G.f$weather.WC, f=1 )
  par( pty="s" ) ; plotYieldsTM( yG1 )
  
  yCn <- fYieldsTM( parMatrix=parMAP, sfiles=sitesettings_filenames,
                    WC=TRUE, list_w=df_C.f$weather.WC, f=n_C.f )
  par( pty="s" ) ; plotYieldsTM( yCn )
  
  yGn <- fYieldsTM( parMatrix=parMAP, sfiles=sitesettings_filenames,
                    WC=TRUE, list_w=df_G.f$weather.WC, f=n_G.f )
  par( pty="s" ) ; plotYieldsTM( yGn )
```
...
```{r}
  ...
  df_C.f[ iC.r2max, c("lat","lon","altitude")]
  df_G.f[ iG.r2max, c("lat","lon","altitude")]
```
...
```{r, fig.height=8}
  ...
  plot  ( df_C.f$altitude, ymean_C.f_condsTM, main="Costa Rica",
  ...
  plot  ( df_G.f$altitude, ymean_G.f_condsTM, main="Guatemala",
          xlab="Alt. (m)", ylab="", ylim=c(0,ymax), pch=20 )
...
```{r, out.width="100%"}
  ...
  fcode_Ci <- 1103 ; df_C.fi <- df_C.f %>% filter( code==fcode_Ci )
  fcode_Gi <- 2009 ; df_G.fi <- df_G.f %>% filter( code==fcode_Gi )
  
  C_C <- df_C.fi$C.soil ; CN_C <- df_C.fi$CN.soil
  C_G <- df_G.fi$C.soil ; CN_G <- df_G.fi$CN.soil
...
```{r, eval=F}
  ...
  for(fi in 1:n_C.f) {
    ...
    parMatrix_C[ c(ip_CNLITT0,ip_CNSOMF0,ip_CNSOMS0), ] <- CN_C
    parMatrix_C[ ip_CSOM0                           , ] <- C_C
    yCfi <- fYieldsTM( parMatrix=parMatrix_C, sfiles=sitesettings_filenames,
                       WC=TRUE, list_w=df_C.f$weather.WC, f=fi )
  ...
  for(fi in 1:n_G.f) {
    ...
    parMatrix_G[ c(ip_CNLITT0,ip_CNSOMF0,ip_CNSOMS0), ] <- CN_G
    parMatrix_G[ ip_CSOM0                           , ] <- C_G
    yGfi <- fYieldsTM( parMatrix=parMatrix_G, sfiles=sitesettings_filenames,
                       WC=TRUE, list_w=df_G.f$weather.WC, f=fi )
...
```{r}
  r2_soil.WC_C <- rep( NA, n_C.f )
  r2_soil.WC_G <- rep( NA, n_G.f )
...
```{r}
  ...
  plot  ( 1:n_G.f, r2_soil.WC_G, col="red", ylim=c(0,1.2*ymax) )
  points( 1:n_C.f, r2_soil.WC_C )
  legend( "topleft",
...
```{r, fig.height=8}
  ...
  plot  ( df_C.f$altitude, ymean_C.f_soil.WC, main="Costa Rica",
          xlab="Alt. (m)", ylab="", ylim=c(0,ymax), pch=20 )
  ...
  plot  ( df_G.f$altitude, ymean_G.f_soil.WC, main="Guatemala",
          xlab="Alt. (m)", ylab="", ylim=c(0,ymax), pch=20 )
