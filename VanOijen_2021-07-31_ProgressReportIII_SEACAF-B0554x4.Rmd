...
```{r}
  dir_farms       <- "data/Farms SEACAF/"
  file_coords.CRI <- "coordinates_CR.csv"
  file_coords.GTM <- "coordinates_G.csv"
  
  dir_WC          <- "weather/WorldClim/"
  dir_WC_prec     <- paste0( dir_WC, "prec/" )
  dir_WC_srad     <- paste0( dir_WC, "srad/" )
  dir_WC_tavg     <- paste0( dir_WC, "tavg/" )
  dir_WC_vapr     <- paste0( dir_WC, "vapr/" )
  dir_WC_wind     <- paste0( dir_WC, "wind/" )
```

```{r}
  df_CRI <- read.csv( paste0( dir_farms, file_coords.CRI ) )
  df_GTM <- read.csv( paste0( dir_farms, file_coords.GTM ) )
  ...
  points_CRI    <- cbind( lon=df_CRI$Lon, lat=df_CRI$Lat )
  points_GTM    <- cbind( lon=df_GTM$Lon, lat=df_GTM$Lat )
  points_CRIGTM <- rbind( points_CRI, points_GTM )
```
...
```{r, fig.cap="\\label{fig:MapFarms}Locations of SEACAF farms in Costa Rica and Guatemala."}
  ...
  plot( worldmap,
        ...
        bg="aliceblue", asp=1 )
  points( points_CRI, col="red", pch=1, cex=0.2 )
  points( points_GTM, col="blue", pch=1, cex=0.2 )
```
...
```{r}
  ...
  r_worldclim_prec_NW  <- getData('worldclim', var='prec' , res=0.5,
                              lon=-92.07, lat=15.09)
  r_worldclim_alt_SE   <- getData('worldclim', var='alt'  , res=0.5,
                              lon=-83.51, lat= 9.58)
  r_worldclim_alt_NW   <- getData('worldclim', var='alt'  , res=0.5,
                              lon=-92.07, lat=15.09)
  tmean_CRI <- raster::extract( r_worldclim_tmean_SE, points_CRI ) / 10
  tmean_GTM <- raster::extract( r_worldclim_tmean_NW, points_GTM ) / 10
  prec_CRI  <- raster::extract( r_worldclim_prec_SE , points_CRI )
  prec_GTM  <- raster::extract( r_worldclim_prec_NW , points_GTM )
  alt_CRI   <- raster::extract( r_worldclim_alt_SE  , points_CRI )
  alt_GTM   <- raster::extract( r_worldclim_alt_NW  , points_GTM )
```
    
```{r}
  par( mfrow=c(2,2) )
  plot  ( alt_CRI, tmean_CRI[,7], ylim=c(0,max(tmean_CRI)) )
  points( alt_CRI, tmean_CRI[,1], col="red" )
  plot  ( alt_CRI, prec_CRI [,7], ylim=c(0,max(prec_CRI )) )
  points( alt_CRI, prec_CRI [,1], col="red" )
  plot  ( alt_GTM, tmean_GTM[,7], ylim=c(0,max(tmean_GTM)) )
  points( alt_GTM, tmean_GTM[,1], col="red" )
  plot  ( alt_GTM, prec_GTM [,7], ylim=c(0,max(prec_GTM )) )
  points( alt_GTM, prec_GTM [,1], col="red" )
```
...
```{r}
  ...
  tavg01 <- raster( paste0( dir_WC_tavg, "wc2.1_30s_tavg_01.tif" ) )
  tavg02 <- raster( paste0( dir_WC_tavg, "wc2.1_30s_tavg_02.tif" ) )
  tavg03 <- raster( paste0( dir_WC_tavg, "wc2.1_30s_tavg_03.tif" ) )
  tavg04 <- raster( paste0( dir_WC_tavg, "wc2.1_30s_tavg_04.tif" ) )
  tavg05 <- raster( paste0( dir_WC_tavg, "wc2.1_30s_tavg_05.tif" ) )
  tavg06 <- raster( paste0( dir_WC_tavg, "wc2.1_30s_tavg_06.tif" ) )
  tavg07 <- raster( paste0( dir_WC_tavg, "wc2.1_30s_tavg_07.tif" ) )
  tavg08 <- raster( paste0( dir_WC_tavg, "wc2.1_30s_tavg_08.tif" ) )
  tavg09 <- raster( paste0( dir_WC_tavg, "wc2.1_30s_tavg_09.tif" ) )
  tavg10 <- raster( paste0( dir_WC_tavg, "wc2.1_30s_tavg_10.tif" ) )
  tavg11 <- raster( paste0( dir_WC_tavg, "wc2.1_30s_tavg_11.tif" ) )
  tavg12 <- raster( paste0( dir_WC_tavg, "wc2.1_30s_tavg_12.tif" ) )
...
```{r}
  prec.data        <- df_CRIGTM 
  prec.data$prec01 <- raster::extract(prec01, points_CRIGTM) / 31
  prec.data$prec02 <- raster::extract(prec02, points_CRIGTM) / 28
  prec.data$prec03 <- raster::extract(prec03, points_CRIGTM) / 31
  prec.data$prec04 <- raster::extract(prec04, points_CRIGTM) / 30
  prec.data$prec05 <- raster::extract(prec05, points_CRIGTM) / 31
  prec.data$prec06 <- raster::extract(prec06, points_CRIGTM) / 30
  prec.data$prec07 <- raster::extract(prec07, points_CRIGTM) / 31
  prec.data$prec08 <- raster::extract(prec08, points_CRIGTM) / 31
  prec.data$prec09 <- raster::extract(prec09, points_CRIGTM) / 30
  prec.data$prec10 <- raster::extract(prec10, points_CRIGTM) / 31
  prec.data$prec11 <- raster::extract(prec11, points_CRIGTM) / 30
  prec.data$prec12 <- raster::extract(prec12, points_CRIGTM) / 31
  
  srad.data        <- df_CRIGTM 
  srad.data$srad01 <- raster::extract(srad01, points_CRIGTM) / 1000
  srad.data$srad02 <- raster::extract(srad02, points_CRIGTM) / 1000
  srad.data$srad03 <- raster::extract(srad03, points_CRIGTM) / 1000
  srad.data$srad04 <- raster::extract(srad04, points_CRIGTM) / 1000
  srad.data$srad05 <- raster::extract(srad05, points_CRIGTM) / 1000
  srad.data$srad06 <- raster::extract(srad06, points_CRIGTM) / 1000
  srad.data$srad07 <- raster::extract(srad07, points_CRIGTM) / 1000
  srad.data$srad08 <- raster::extract(srad08, points_CRIGTM) / 1000
  srad.data$srad09 <- raster::extract(srad09, points_CRIGTM) / 1000
  srad.data$srad10 <- raster::extract(srad10, points_CRIGTM) / 1000
  srad.data$srad11 <- raster::extract(srad11, points_CRIGTM) / 1000
  srad.data$srad12 <- raster::extract(srad12, points_CRIGTM) / 1000
  
  tavg.data        <- df_CRIGTM 
  tavg.data$tavg01 <- raster::extract(tavg01, points_CRIGTM) / 10
  tavg.data$tavg02 <- raster::extract(tavg02, points_CRIGTM) / 10
  tavg.data$tavg03 <- raster::extract(tavg03, points_CRIGTM) / 10
  tavg.data$tavg04 <- raster::extract(tavg04, points_CRIGTM) / 10
  tavg.data$tavg05 <- raster::extract(tavg05, points_CRIGTM) / 10
  tavg.data$tavg06 <- raster::extract(tavg06, points_CRIGTM) / 10
  tavg.data$tavg07 <- raster::extract(tavg07, points_CRIGTM) / 10
  tavg.data$tavg08 <- raster::extract(tavg08, points_CRIGTM) / 10
  tavg.data$tavg09 <- raster::extract(tavg09, points_CRIGTM) / 10
  tavg.data$tavg10 <- raster::extract(tavg10, points_CRIGTM) / 10
  tavg.data$tavg11 <- raster::extract(tavg11, points_CRIGTM) / 10
  tavg.data$tavg12 <- raster::extract(tavg12, points_CRIGTM) / 10
  
  vapr.data        <- df_CRIGTM 
  vapr.data$vapr01 <- raster::extract(vapr01, points_CRIGTM)
  vapr.data$vapr02 <- raster::extract(vapr02, points_CRIGTM)
  vapr.data$vapr03 <- raster::extract(vapr03, points_CRIGTM)
  vapr.data$vapr04 <- raster::extract(vapr04, points_CRIGTM)
  vapr.data$vapr05 <- raster::extract(vapr05, points_CRIGTM)
  vapr.data$vapr06 <- raster::extract(vapr06, points_CRIGTM)
  vapr.data$vapr07 <- raster::extract(vapr07, points_CRIGTM)
  vapr.data$vapr08 <- raster::extract(vapr08, points_CRIGTM)
  vapr.data$vapr09 <- raster::extract(vapr09, points_CRIGTM)
  vapr.data$vapr10 <- raster::extract(vapr10, points_CRIGTM)
  vapr.data$vapr11 <- raster::extract(vapr11, points_CRIGTM)
  vapr.data$vapr12 <- raster::extract(vapr12, points_CRIGTM)

  wind.data        <- df_CRIGTM 
  wind.data$wind01 <- raster::extract(wind01, points_CRIGTM)
  wind.data$wind02 <- raster::extract(wind02, points_CRIGTM)
  wind.data$wind03 <- raster::extract(wind03, points_CRIGTM)
  wind.data$wind04 <- raster::extract(wind04, points_CRIGTM)
  wind.data$wind05 <- raster::extract(wind05, points_CRIGTM)
  wind.data$wind06 <- raster::extract(wind06, points_CRIGTM)
  wind.data$wind07 <- raster::extract(wind07, points_CRIGTM)
  wind.data$wind08 <- raster::extract(wind08, points_CRIGTM)
  wind.data$wind09 <- raster::extract(wind09, points_CRIGTM)
  wind.data$wind10 <- raster::extract(wind10, points_CRIGTM)
  wind.data$wind11 <- raster::extract(wind11, points_CRIGTM)
  wind.data$wind12 <- raster::extract(wind12, points_CRIGTM)
```

```{r}
  par( mfrow=c(2,2) )

  plot( prec.data$prec01, c(prec_CRI[,1],prec_GTM[,1]) )
  plot( prec.data$prec07, c(prec_CRI[,7],prec_GTM[,7]) )
  plot( tavg.data$tavg01, c(tmean_CRI[,1],tmean_GTM[,1]) )
  plot( tavg.data$tavg07, c(tmean_CRI[,7],tmean_GTM[,7]) )
```
...
```{r}
  df_CRI.full <- df_CRI %>%
    mutate( Alt = alt_CRI )
  df_GTM.full <- df_GTM %>% mutate( Alt = alt_GTM )
```
