...
Various model outputs for the 32 experimental treatments are shown in Fig. \ref{fig:TM}. 
...
In this section, we study the impact of adding banana plants on various outputs from the model. The reference simulations are those that we looked at above, i.e. those for the long-term experiments in Turrialba (Costa Rica) and Masatepe (Nicaragua), which did not include any fruit trees.
...
WorldClim provides data for both climate (30-year averages) and weather (2000-2018).
...
# Combining model inputs in an R-dataframe and preliminary data analysis
...
Farm yield observations vs. simulations where the model in case of 3 or more tree included banana.
...
## Yield prediction and coffee harvest date

- MvO:
  - "The CAF model simulates time to maturation (from flowering to harvesting) as a function of temperature, and temperature is decreased under shade. So a partly shaded field has two harvesting dates: an early one for the coffee plants not covered by the trees, and a later one for the plants under shade. But even for plants under very cool conditions, the model still forces the harvest to happen before the end of the calendar year. That’s an assumption of convenience: the model code would be more complicated if we allow harvesting into the new year, especially if it could overlap with the next flowering. [Likewise, simulating flowering and harvesting to happen on single days (per shade level) rather than over extended periods keeps the code simple.]
  - So to simulate the harvest season of 2019 we need no weather data for 2020 even if in reality the harvesting period extended to 2020."
- JH:
  - "For high altitude sites with lower temperatures forcing harvest at the end of the calendar year might lead to an underestimation of harvest. Likewise in high dry areas such as Antigua Guatemala harvest may not be until March and flowering in May. I appreciate the convenience of keeping things in the calendar year but it might become a significant source of error in these conditions."
