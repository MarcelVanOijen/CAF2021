...
- We calculated dry coffee bean yield (the CAF2021 variable) as reported fresh yield divided by 3. The Nicaragua experiment showed that the "factor de conversión de café cereza a café oro" was variable with an average of 7.74 [@Sepulveda_2016_Efecto]. A more common value may be 6. and if we further assume that 'oro' is half the value of dry yield, then the conversion factor from fresh to dry should be 3.
...
```{r, eval=F}
  dir.f      <- "data/Farms SEACAF/" ; file_typol.f <- "typology.XLSX" 
...
```{r, eval=F}
  # df_C.f <- df_C.f %>% mutate( C.soil = C.soil * 4/5 ) # Subsoil 20% of C
  # df_G.f <- df_G.f %>% mutate( C.soil = C.soil * 4/5 ) # Subsoil 20% of C
```

```{r, eval=F}
  # save( df_C.f, file="df_C.f_2021-09-24.rda" )
  # save( df_G.f, file="df_G.f_2021-09-24.rda" )
```
...
```{r, eval=F}
  # fileTM_MAP   <- paste0( "BC/BC_RESULTS_Turrialba-Masatepe/BC_2021-09-17_6e4/",
  #                         "CAF2021_parMAP_06_45.rds" )
  # matMAP       <- readRDS( fileTM_MAP )
  # 
  # dir.params   <- "parameters/"
  # file_par_def <- paste0( dir.params, "parameters_default_parMAP_2021-09-17.txt" )
  # file.create( file_par_def )
  # write.table( matMAP, file=file_par_def, sep="\t", col.names=NA )
```
...
```{r, eval=T}
  file_RData <- paste0( "BC/BC_RESULTS_Turrialba-Masatepe/BC_2021-09-17_6e4/",
                        "BC_Turrialba_Masatepe_06_45.RData" )
  load( file_RData )
  source( sitesettings_filenames[1] )

  aa <- 1 + 4 * (2-1)/(4-1) # Typical prior: if mode=2, min=1, max=4
  bb <- 6 - aa
  pp <- 1 + rbeta(1e5,aa,bb)*(4-1)
  mean(pp) ## should be 1 + (4-1) * (aa/(aa+bb)) = 13/6
  # mode(pp) should be 1 + (4-1) * (aa-1)/(aa+bb-2) = 2
  sd(pp) / mean(pp)
  sqrt(aa*bb) / (aa+bb) / sqrt(aa+bb+1) * (4-1) / (13/6) # prior cv = 25.51 %
```

```{r}
  file_parModes <- paste0( "BC/BC_RESULTS_Turrialba-Masatepe/BC_2021-09-17_6e4/",
                           "CAF2021_parModes_06_45.txt" )
  df_parModes   <- read.table( file_parModes, sep="\t", header=T, row.names=1 )
  # knitr::kable( cbind( names_params, df_parModes[,"CVPost_1"] ) )
  # iBC           <- which(df_parModes[,"CVPost_1"]!=0)
  iCV           <- which( startsWith( colnames(df_parModes), "CVPost_" ) )
  # summary(         df_parModes[iBC,iCV]   ) # Summary for calibrated parameters
  summary( unlist( df_parModes[   ,iCV] ) ) # Summary for all parameters
```

```{r, eval=T, cache=T}
# Calculate model output for a sample from the posterior
# Take a sample (of size nSample) from the chain generated using MCMC
  mean_s <- cv_s <- matrix(NA,nrow=nSites,ncol=NOUT)
  for (s in 1:nSites) {
    params          <- list_params        [[s]]
    matrix_weather  <- list_matrix_weather[[s]]
    calendar_fert   <- list_calendar_fert [[s]]
    calendar_prunC  <- list_calendar_prunC[[s]] 
    calendar_prunT  <- list_calendar_prunT[[s]]
    calendar_thinT  <- list_calendar_thinT[[s]]  
    NDAYS           <- list_NDAYS         [[s]]       
    ...
    for (j in seq(nBurnin+nStep, nChain, nStep)) {
      ...
      outputSample[ii,,] <- run_model( params, matrix_weather,
                                       calendar_fert, calendar_prunC,
	              	                     calendar_prunT, calendar_thinT, NDAYS )
    } # end of sample loop
  mean_s[s,] <- sapply( 1:NOUT, function(i) {
      mean(outputSample[,NDAYS,i],na.rm=T)
    } )
  cv_s[s,] <- sapply( 1:NOUT, function(i) {
      sd  (outputSample[,NDAYS,i],na.rm=T) /
      mean(outputSample[,NDAYS,i],na.rm=T)
    } )

  colnames(mean_s) <- colnames(cv_s) <- outputNames
  }
  
  cbind (summary(mean_s[,c("Shade_f","harvDM_f_hay",
                           "D_Csoil_f_hay","Nleaching_f_hay")]) )
  cbind (summary(cv_s  [,c("Shade_f","harvDM_f_hay",
                           "D_Csoil_f_hay","Nleaching_f_hay")]) )
```
...
```{r}
  vars_Summary <- c(
    ...
    "harvDMST_3_hay" , "harvDMPT_f_hay"  , "harvDMST_f"    , "harvCST_3",
    "D_Nsoil"        ,
    "D_Csoil"        , "D_C"            , "D_CT"         , "D_Csys"     ,
    "Shade_f"        , "Shade_fa"       , "Shade_fb"     ,
    "Nemission_fb"   , "NfixT_fb"       , "Nmineralis_fb",
    "Nleaching_fb"   , "Crunoff_fb"     ,
    "Csoil_fa"       , "Csoil_fb"       ,
...
```{r}
  outputSummary <- function( output ) {
    ...
    iNmineralis <- which( outputNames=="Nmineralisation_f") # kg N m-2 d-1
    ...
    T3 <- 0 ; if(output[1,itreedens_3]>0) T3 <- 1
           ...
           fOut[,"harvDMPT_f_hay"]  <- sum ( output[     ,ihCPT_f ] ) * c.DM * c.hay
           fOut[,"harvDMST_f"]      <- sum ( output[     ,ihCST_f ] ) * c.DM * c.hay
    ...
    fOut[,"Nmineralis_fb"] <- outputFinalYears[iNmineralis] * 1e4 * 365
...
```{r list_outF_C, cache=F}
  ...
  for(fi in 1:n_C.f) {
    ...
    for(iF in 1:nF_C) {
      params.2         <- set_par("NFERTMULT",0.5)
      params.3         <- set_par("SHADETARGETMULT",0.1)
      params.4         <- set_par("SHADETARGETMULT",0.1)
        # iPRUNTARGET <- which( startsWith( names_params, "PRUNTARGET(" ) )
        iTREEDENS0  <- which( startsWith( names_params, "TREEDENS0(" ) )
      # params.4[ iPRUNTARGET ] <- 0
      params.4[ iTREEDENS0 ]  <- params.4[ iTREEDENS0 ] * 0.2
...
```{r list_outF_C_SA, eval=F, cache=F}
  list_outF_C_SA <- vector( "list", n_C.f )
  ...
  for(fi in 1:n_C.f) {
    ...
    npar                  <- length(params)
    mat_outF              <- matrix( NA, npar, length(vars_Summary) )
    colnames( mat_outF )  <- vars_Summary
    for(pi in 1:npar) {
      paramsS     <- params
      paramsS[pi] <- params[pi] * 0.9
      outF <- run_model( paramsS, matrix_weather, calendar_fert,
        calendar_prunC, calendar_prunT, calendar_thinT, NDAYS )
      mat_outF[pi,] <- outputSummary( outF )
      if(startsWith(names_params[pi],"PRUNTARGET(")) mat_outF[pi,] <- NA
      if(startsWith(names_params[pi],"THINTARGET(")) mat_outF[pi,] <- NA
    }
    list_outF_C_SA[[fi]] <- mat_outF
  }

  mat_outF_C_SA <- mat_outF_C_SA.minf <- mat_outF_C_SA.maxf <-
    matrix( NA, nrow=npar, ncol=length(vars_Summary) )
  rownames(mat_outF_C_SA) <- names_params
  colnames(mat_outF_C_SA) <- vars_Summary
  for(pi in 1:npar) {
    for(vi in 1:length(vars_Summary)) {
      S_pivi.f <- rep( NA, n_C.f )
      for(fi in 1:n_C.f) {
        S_pivi.f[fi] <- -10 *
          ( list_outF_C_SA[[fi]][pi,vi]/list_outF_C[[fi]][1,vi+1] - 1 )
      }
      mat_outF_C_SA     [pi,vi] <- round( mean( S_pivi.f, na.rm=T ), 2 )
      mat_outF_C_SA.minf[pi,vi] <- round( min ( S_pivi.f, na.rm=T ), 2 )
      mat_outF_C_SA.maxf[pi,vi] <- round( max ( S_pivi.f, na.rm=T ), 2 )
    }
  }
  
  Ssummary_C.v <- sapply( 1:length(vars_Summary), function(vi){
    range( mat_outF_C_SA[,vi], na.rm=T ) } )
  colnames( Ssummary_C.v ) <- vars_Summary

  Ssummary_C.vpmax <- sapply( 1:length(vars_Summary), function(vi){
    ipmax <- which.max( abs( mat_outF_C_SA[,vi] ) )
    return( c( names(ipmax), mat_outF_C_SA.minf[ipmax,vi],
               mat_outF_C_SA[ipmax,vi], mat_outF_C_SA.maxf[ipmax,vi] ) ) } )

  table_Ssummary_C <- cbind( t(Ssummary_C.v), t(Ssummary_C.vpmax) )
  colnames( table_Ssummary_C ) <- c( "min_Smeanf(p)", "max_Smeanf(p)",
                                     "P(maxabs_S)",
                                     "min_SP(f)","mean_SP(f)","max_SP(f)" )
  
  knitr::kable( table_Ssummary_C )
```
...
```{r}
  dfoutFSummary <- function( df=df_C.f_sim, Fi=1 ){
    ...
    F.v <- matrix( NA, nf, nv ) ; colnames( F.v ) <- vnames
    for(fi in 1:nf) F.v[fi,] <- df$outF[[fi]][Fi,]
    return( F.v )
  }
```
...
```{r}
  dfparFSummary <- function( df=df_C.f_sim, Fi=1 ){
    ...
    F.v    <- matrix( NA, nf, nv ) ; colnames( F.v ) <- vnames
    for(fi in 1:nf) F.v[fi,] <- df$parF[[fi]][Fi,]
    return( F.v )
  }
```
...
```{r}
  tableY_C <- ( rbind(
    ...
    rep( "", 8 ),
    c( "Sim.4", "18/19", summary( oF4_C[,"harvDM_f_ha1819"], digits=3 ) ),
    c( ""     , "19/20", summary( oF4_C[,"harvDM_f_ha1920"], digits=3 ) ) ) )
...
```{r}
  tableCA_C1 <- ( rbind(
    c( "Erythina" , summary( oF1_C[if_C.1E,"CAtree_1b"], digits=3 ) ),
    c( "Inga"     , summary( oF1_C[if_C.1I,"CAtree_1b"], digits=3 ) ),
    c( "Banana"   , summary( oF1_C[if_C.2B,"CAtree_2b"], digits=3 ) ),
    c( "Avocado"  , summary( oF1_C[if_C.2A,"CAtree_2b"], digits=3 ) ),
    c( "Grevillea", summary( oF1_C[if_C.3G,"CAtree_3b"], digits=3 ) ),
    c( "Cordia"   , summary( oF1_C[if_C.3C,"CAtree_3b"], digits=3 ) ) ) )
  colnames(tableCA_C1)[1] <- "C1: CAtree (m2)"
  knitr::kable( tableCA_C1 )

  tableH_C1 <- ( rbind(
    c( "Erythina" , summary( oF1_C[if_C.1E,"H_1b"], digits=3 ) ),
    c( "Inga"     , summary( oF1_C[if_C.1I,"H_1b"], digits=3 ) ),
    c( "Banana"   , summary( oF1_C[if_C.2B,"H_2b"], digits=3 ) ),
    c( "Avocado"  , summary( oF1_C[if_C.2A,"H_2b"], digits=3 ) ),
    c( "Grevillea", summary( oF1_C[if_C.3G,"H_3b"], digits=3 ) ),
    c( "Cordia"   , summary( oF1_C[if_C.3C,"H_3b"], digits=3 ) ) ) )
...
```{r XX14C, fig.height=8, fig.cap="\\label{fig:XX14C}Observed vs. simulated yields in 2018-19 and 2019-20 at the farms in Costa Rica."}
  par( mfrow=c(4,3), mar=c(4,3,2,2), mgp=c(2,1,0) )
  ...
  x=oF4_C[,"harvDM_f_ha1819"] ; y=df_C.f$yield1819
    plotxylm("Yields Costa Rica 18/19", orig=F)
  x=oF4_C[,"harvDM_f_ha1920"] ; y=df_C.f$yield1920
    plotxylm("Yields Costa Rica 19/20", orig=F)
  x=c( oF4_C[,"harvDM_f_ha1819"], oF4_C[,"harvDM_f_ha1920"] )
  y=c( df_C.f$yield1819         , df_C.f$yield1920          )
    plotxylm("Yields Costa Rica 2 yrs", orig=F)
```

```{r list_outF_G, cache=F}
...
```{r list_outF_G_SA, eval=F, cache=F}
  list_outF_G_SA <- vector( "list", n_G.f )
  ...
  for(fi in 1:n_G.f) {
    ...
    list_outF_G_SA[[fi]] <- mat_outF
  }

  mat_outF_G_SA <- mat_outF_G_SA.minf <- mat_outF_G_SA.maxf <-
    matrix( NA, nrow=npar, ncol=length(vars_Summary) )
  rownames(mat_outF_G_SA) <- names_params
  colnames(mat_outF_G_SA) <- vars_Summary
  for(pi in 1:npar) {
    for(vi in 1:length(vars_Summary)) {
      S_pivi.f <- rep( NA, n_G.f )
      for(fi in 1:n_G.f) {
        S_pivi.f[fi] <- -10 *
          ( list_outF_G_SA[[fi]][pi,vi]/list_outF_G[[fi]][1,vi+1] - 1 )
      }
      mat_outF_G_SA     [pi,vi] <- round( mean( S_pivi.f, na.rm=T ), 2 )
      mat_outF_G_SA.minf[pi,vi] <- round( min ( S_pivi.f, na.rm=T ), 2 )
      mat_outF_G_SA.maxf[pi,vi] <- round( max ( S_pivi.f, na.rm=T ), 2 )
    }
  }
  
  Ssummary_G.v <- sapply( 1:length(vars_Summary), function(vi){
    range( mat_outF_G_SA[,vi], na.rm=T ) } )
  colnames( Ssummary_G.v ) <- vars_Summary

  Ssummary_G.vpmax <- sapply( 1:length(vars_Summary), function(vi){
    ipmax <- which.max( abs( mat_outF_G_SA[,vi] ) )
    return( c( names(ipmax), mat_outF_G_SA.minf[ipmax,vi],
               mat_outF_G_SA[ipmax,vi], mat_outF_G_SA.maxf[ipmax,vi] ) ) } )

  table_Ssummary_G <- cbind( t(Ssummary_G.v), t(Ssummary_G.vpmax) )
  colnames( table_Ssummary_G ) <- c( "min_Smeanf(p)", "max_Smeanf(p)",
                                     "P(maxabs_S)",
                                     "min_SP(f)","mean_SP(f)","max_SP(f)" )
  
  knitr::kable( table_Ssummary_G )
```
...
```{r}
  tableY_G <- ( rbind(
    ...
    rep( "", 8 ),
    c( "Sim.4", "18/19", summary( oF4_G[,"harvDM_f_ha1819"], digits=3 ) ),
    c( ""     , "19/20", summary( oF4_G[,"harvDM_f_ha1920"], digits=3 ) ) ) )
...
```{r}
  tableCA_G1 <- ( rbind(
    c( "Inga"     , summary( oF1_G[if_G.1I,"CAtree_1b"], digits=3 ) ),
    c( "Banana"   , summary( oF1_G[if_G.2B,"CAtree_2b"], digits=3 ) ),
    c( "Avocado"  , summary( oF1_G[if_G.2A,"CAtree_2b"], digits=3 ) ),
    c( "Grevillea", summary( oF1_G[if_G.3G,"CAtree_3b"], digits=3 ) ),
    c( "Cordia"   , summary( oF1_G[if_G.3C,"CAtree_3b"], digits=3 ) ) ) )
  colnames(tableCA_G1)[1] <- "G1: CAtree (m2)"
  knitr::kable( tableCA_G1 )

  tableH_G1 <- ( rbind(
    c( "Inga"     , summary( oF1_G[if_G.1I,"H_1b"], digits=3 ) ),
    c( "Banana"   , summary( oF1_G[if_G.2B,"H_2b"], digits=3 ) ),
    c( "Avocado"  , summary( oF1_G[if_G.2A,"H_2b"], digits=3 ) ),
    c( "Grevillea", summary( oF1_G[if_G.3G,"H_3b"], digits=3 ) ),
    c( "Cordia"   , summary( oF1_G[if_G.3C,"H_3b"], digits=3 ) ) ) )
...
```{r XX14G, fig.height=8, fig.cap="\\label{fig:XX14G}Observed vs. simulated yields in 2018-19 and 2019-20 at the farms in Guatemala."}
  par( mfrow=c(4,3), mar=c(4,3,2,2), mgp=c(2,1,0) )
  ...
  x=oF4_G[,"harvDM_f_ha1819"] ; y=df_G.f$yield1819
    plotxylm("Yields Guatemala 18/19" , orig=F)
  x=oF4_G[,"harvDM_f_ha1920"] ; y=df_G.f$yield1920
    plotxylm("Yields Guatemala 19/20" , orig=F)
  x=c( oF4_G[,"harvDM_f_ha1819"], oF4_G[,"harvDM_f_ha1920"] )
  y=c( df_G.f$yield1819         , df_G.f$yield1920          )
    plotxylm("Yields Guatemala 2 yrs" , orig=F)
```
...
```{r XX21, fig.height=5, fig.cap="\\label{fig:XX21}Final water limitation of the three tree classes vs. N-fertilisation (top row) and vs. distance from the easternmost farm (bottom row). Black: Costa Rica. Red: Guatemala."}
  ...
  yv <- "fTranT_1b" ; plotimpacts_CG( FC=oF1_C, FG=oF1_G, yvar=yv )
  yv <- "fTranT_2b" ; plotimpacts_CG( FC=oF1_C, FG=oF1_G, yvar=yv )
  yv <- "fTranT_3b" ; plotimpacts_CG( FC=oF1_C, FG=oF1_G, yvar=yv )

  yv <- "fTranT_1b" ; plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="dist", yvar=yv )
  yv <- "fTranT_2b" ; plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="dist", yvar=yv )
  yv <- "fTranT_3b" ; plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="dist", yvar=yv )
```
...
```{r XX22, fig.height=5, fig.cap="\\label{fig:XX22}Final N-limitation of the three tree classes vs. N-fertilisation (top row) and vs. distance from the easternmost farm (bottom row). Black: Costa Rica. Red: Guatemala."}
  ...
  yv <- "fNgrowth_1b" ; plotimpacts_CG( FC=oF1_C, FG=oF1_G, yvar=yv )
  yv <- "fNgrowth_2b" ; plotimpacts_CG( FC=oF1_C, FG=oF1_G, yvar=yv )
  yv <- "fNgrowth_3b" ; plotimpacts_CG( FC=oF1_C, FG=oF1_G, yvar=yv )
...
```{r XX24, fig.height=5, fig.cap="\\label{fig:XX24}Costa Rica: Tree height vs. N-fertilisation (top row) and distance from easternmost farm (bottom row). Black: Costa Rica. Red: Guatemala."}
  ...
  yv <- "H_1b" ; plotimpacts_CG( FC=oF1_C, FG=oF1_G, yvar=yv )
  yv <- "H_2b" ; plotimpacts_CG( FC=oF1_C, FG=oF1_G, yvar=yv )
  yv <- "H_3b" ; plotimpacts_CG( FC=oF1_C, FG=oF1_G, yvar=yv )

  yv <- "H_1b" ; plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="dist", yvar=yv )
  yv <- "H_2b" ; plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="dist", yvar=yv )
  yv <- "H_3b" ; plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="dist", yvar=yv )
```
...
```{r XX13, fig.height=3, fig.cap="\\label{fig:XX13}Yields in 2019-20 vs. yield in 2018-19."}
  ...
  plot  ( x_C, y_C, main="Simulated yields.1\n(kg DM ha-1 y-1)",
          xlab="2018-19", ylab="2019-20")
  points( x_G, y_G, col="red" )
  y.lm_C <- lm( y_C ~ x_C ) ; r2_C <- round( summary(y.lm_C)$r.squared, 2 )
  y.lm_G <- lm( y_G ~ x_G ) ; r2_G <- round( summary(y.lm_G)$r.squared, 2 )
  ...
  plot  ( x_C, y_C, main="Simulated yields.2\n(kg DM ha-1 y-1)",
          xlab="2018-19", ylab="2019-20")
  points( x_G, y_G, col="red" )
  y.lm_C <- lm( y_C ~ x_C ) ; r2_C <- round( summary(y.lm_C)$r.squared, 2 )
  y.lm_G <- lm( y_G ~ x_G ) ; r2_G <- round( summary(y.lm_G)$r.squared, 2 )
  ...
  x_C <- oF3_C[,"harvDM_f_ha1819"] ; y_C <- oF3_C[,"harvDM_f_ha1920"]
  x_G <- oF3_G[,"harvDM_f_ha1819"] ; y_G <- oF3_G[,"harvDM_f_ha1920"]
  plot  ( x_C, y_C, main="Simulated yields.3\n(kg DM ha-1 y-1)",
          xlab="2018-19", ylab="2019-20")
  points( x_G, y_G, col="red" )
  y.lm_C <- lm( y_C ~ x_C ) ; r2_C <- round( summary(y.lm_C)$r.squared, 2 )
  y.lm_G <- lm( y_G ~ x_G ) ; r2_G <- round( summary(y.lm_G)$r.squared, 2 )
  ...
  x_C <- oF4_C[,"harvDM_f_ha1819"] ; y_C <- oF4_C[,"harvDM_f_ha1920"]
  x_G <- oF4_G[,"harvDM_f_ha1819"] ; y_G <- oF4_G[,"harvDM_f_ha1920"]
  plot  ( x_C, y_C, main="Simulated yields.4\n(kg DM ha-1 y-1)",
          xlab="2018-19", ylab="2019-20")
  points( x_G, y_G, col="red" )
  y.lm_C <- lm( y_C ~ x_C ) ; r2_C <- round( summary(y.lm_C)$r.squared, 2 )
  y.lm_G <- lm( y_G ~ x_G ) ; r2_G <- round( summary(y.lm_G)$r.squared, 2 )
...
```{r}
  ...
  sim <- sapply( 1:length(if_G.type13), function(s) {
    list_outF_G[if_G.type13[s]][[1]][,"Shade_f"][1] } )
  obs <- df_G.f$shade[if_G.type13]

  sim <- sapply( 1:length(if_G.type13), function(s) {
    list_outF_G[if_G.type13[s]][[1]][,"harvDM_f_hay"][1] } )
  obs <- df_G.f$yield1920[if_G.type13]
```

```{r XX31obs, fig.height=5.5, fig.cap="\\label{fig:XX31obs}Observations for 4 farm types in Costa Rica. Farm type codes ranging from 1 = very low to 4 = high: first digit is productivity, second is shade."}
...
```{r XX31sim, fig.height=5.5, fig.cap="\\label{fig:XX31sim}Simulations for 4 farm types in Costa Rica. Farm type codes ranging from 1 = very low to 4 = high: first digit is productivity, second is shade."}
...
```{r XX31F1234, fig.height=7.5, fig.cap="\\label{fig:XX31F1234}Four different simulations (blue) for 4 farm types in Costa Rica. Farm type codes ranging from 1 = very low to 4 = high: first digit is productivity, second is shade."}
  par( mfrow=c(5,4), mar=c(1,2,1,1) )

  ysim    <- "Shade_fb"
  ysim1.12 <- oF1_C[ if_C.type12, ysim ] ; ysim1.24 <- oF1_C[ if_C.type24, ysim ]
  ysim1.32 <- oF1_C[ if_C.type32, ysim ] ; ysim1.43 <- oF1_C[ if_C.type43, ysim ]
  ysim2.12 <- oF2_C[ if_C.type12, ysim ] ; ysim2.24 <- oF2_C[ if_C.type24, ysim ]
  ysim2.32 <- oF2_C[ if_C.type32, ysim ] ; ysim2.43 <- oF2_C[ if_C.type43, ysim ]
  ysim3.12 <- oF3_C[ if_C.type12, ysim ] ; ysim3.24 <- oF3_C[ if_C.type24, ysim ]
  ysim3.32 <- oF3_C[ if_C.type32, ysim ] ; ysim3.43 <- oF3_C[ if_C.type43, ysim ]
  ysim4.12 <- oF4_C[ if_C.type12, ysim ] ; ysim4.24 <- oF4_C[ if_C.type24, ysim ]
  ysim4.32 <- oF4_C[ if_C.type32, ysim ] ; ysim4.43 <- oF4_C[ if_C.type43, ysim ]
  ysim1.   <- list( ysim1.12, ysim1.24, ysim1.32, ysim1.43 )
  ysim2.   <- list( ysim2.12, ysim2.24, ysim2.32, ysim2.43 )
  ysim3.   <- list( ysim3.12, ysim3.24, ysim3.32, ysim3.43 )
  ysim4.   <- list( ysim4.12, ysim4.24, ysim4.32, ysim4.43 )
  boxplot( ysim1., names=rep("",4), main="Shade 1", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim2., names=rep("",4), main="Shade 2", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim3., names=rep("",4), main="Shade 3", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim4., names=rep("",4), main="Shade 4", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  ...
  ysim3.   <- list( ysim3.12, ysim3.24, ysim3.32, ysim3.43 )
  ysim4.   <- list( ysim4.12, ysim4.24, ysim4.32, ysim4.43 )
  boxplot( ysim1., names=rep("",4), main="Yield 1", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim2., names=rep("",4), main="Yield 2", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim3., names=rep("",4), main="Yield 3", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim4., names=rep("",4), main="Yield 4", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  ...
  ysim3.   <- list( ysim3.12, ysim3.24, ysim3.32, ysim3.43 )
  ysim4.   <- list( ysim4.12, ysim4.24, ysim4.32, ysim4.43 )
  boxplot( ysim1., names=rep("",4), main="D_Csoil 1", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim2., names=rep("",4), main="D_Csoil 2", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim3., names=rep("",4), main="D_Csoil 3", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim4., names=rep("",4), main="D_Csoil 4", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  ...
  ysim3.   <- list( ysim3.12, ysim3.24, ysim3.32, ysim3.43 )
  ysim4.   <- list( ysim4.12, ysim4.24, ysim4.32, ysim4.43 )
  boxplot( ysim1., names=rep("",4), main="N-leaching 1", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,150))
  boxplot( ysim2., names=rep("",4), main="N-leaching 2", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,150))
  boxplot( ysim3., names=rep("",4), main="N-leaching 3", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,150))
  boxplot( ysim4., names=rep("",4), main="N-leaching 4", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,150))
  ...
  ysim3.   <- list( ysim3.12, ysim3.24, ysim3.32, ysim3.43 )
  ysim4.   <- list( ysim4.12, ysim4.24, ysim4.32, ysim4.43 )
  boxplot( ysim1., names=c(1.2, 2.4, 3.2, 4.3), main="C-runoff 1", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,600))
  boxplot( ysim2., names=c(1.2, 2.4, 3.2, 4.3), main="C-runoff 2", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,600))
  boxplot( ysim3., names=c(1.2, 2.4, 3.2, 4.3), main="C-runoff 3", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,600))
  boxplot( ysim4., names=c(1.2, 2.4, 3.2, 4.3), main="C-runoff 4", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,600))
```

```{r XX31F1234diff, fig.height=7.5, fig.cap="\\label{fig:XX31F1234diff}Four different simulations (blue) for 4 farm types in Costa Rica. Farm type codes ranging from 1 = very low to 4 = high: first digit is productivity, second is shade."}
  par( mfrow=c(5,4), mar=c(1,2,1,1) )
  ...
  ysim4.12 <- oF4_C[ if_C.type12, ysim ] ; ysim4.24 <- oF4_C[ if_C.type24, ysim ]
  ysim4.32 <- oF4_C[ if_C.type32, ysim ] ; ysim4.43 <- oF4_C[ if_C.type43, ysim ]
  ysim1.   <- list( ysim1.12         , ysim1.24         ,
                    ysim1.32         , ysim1.43 )
  ysim2.   <- list( ysim2.12-ysim1.12, ysim2.24-ysim1.24,
                    ysim2.32-ysim1.32, ysim2.43-ysim1.43 )
  ysim3.   <- list( ysim3.12-ysim1.12, ysim3.24-ysim1.24,
                    ysim3.32-ysim1.32, ysim3.43-ysim1.43 )
  ysim4.   <- list( ysim4.12-ysim1.12, ysim4.24-ysim1.24,
                    ysim4.32-ysim1.32, ysim4.43-ysim1.43 )
  boxplot( ysim1., names=rep("",4), main="Shade 1", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim2., names=rep("",4), main="Shade 2", col="blue",
           cex.main=0.7, cex.axis=0.7, ylim=c(-0.6,0.6))
  boxplot( ysim3., names=rep("",4), main="Shade 3", col="blue",
           cex.main=0.7, cex.axis=0.7, ylim=c(-0.6,0.6))
  boxplot( ysim4., names=rep("",4), main="Shade 4", col="blue",
           cex.main=0.7, cex.axis=0.7, ylim=c(-0.6,0.6))
  ...
  boxplot( ysim1., names=rep("",4), main="Yield 1", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim2., names=rep("",4), main="Yield 2", col="blue",
           cex.main=0.7, cex.axis=0.7, ylim=c(-3000,3000))
  boxplot( ysim3., names=rep("",4), main="Yield 3", col="blue",
           cex.main=0.7, cex.axis=0.7, ylim=c(-3000,3000))
  boxplot( ysim4., names=rep("",4), main="Yield 4", col="blue",
           cex.main=0.7, cex.axis=0.7, ylim=c(-3000,3000))
  ...
  boxplot( ysim1., names=rep("",4), main="D_Csoil 1", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim2., names=rep("",4), main="D_Csoil 2", col="blue",
           cex.main=0.7, cex.axis=0.7, ylim=c(-1500,1500))
  boxplot( ysim3., names=rep("",4), main="D_Csoil 3", col="blue",
           cex.main=0.7, cex.axis=0.7, ylim=c(-1500,1500))
  boxplot( ysim4., names=rep("",4), main="D_Csoil 4", col="blue",
           cex.main=0.7, cex.axis=0.7, ylim=c(-1500,1500))
  ...
  boxplot( ysim1., names=rep("",4), main="N-leaching 1", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,150))
  boxplot( ysim2., names=rep("",4), main="N-leaching 2", col="blue",
           cex.main=0.7, cex.axis=0.7, ylim=c(-100,100))
  boxplot( ysim3., names=rep("",4), main="N-leaching 3", col="blue",
           cex.main=0.7, cex.axis=0.7, ylim=c(-100,100))
  boxplot( ysim4., names=rep("",4), main="N-leaching 4", col="blue",
           cex.main=0.7, cex.axis=0.7, ylim=c(-100,100))
  ...
  boxplot( ysim1., names=c(1.2, 2.4, 3.2, 4.3), main="C-runoff 1", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,600))
  boxplot( ysim2., names=c(1.2, 2.4, 3.2, 4.3), main="C-runoff 2", col="blue",
           cex.main=0.7, cex.axis=0.7, ylim=c(-300,300))
  boxplot( ysim3., names=c(1.2, 2.4, 3.2, 4.3), main="C-runoff 3", col="blue",
           cex.main=0.7, cex.axis=0.7, ylim=c(-300,300))
  boxplot( ysim4., names=c(1.2, 2.4, 3.2, 4.3), main="C-runoff 4", col="blue",
           cex.main=0.7, cex.axis=0.7, ylim=c(-300,300))
```

```{r XX32obs, fig.height=5.5, fig.cap="\\label{fig:XX32obs}Observations for 4 farm types in Guatemala. Farm type codes ranging from 1 = very low to 4 = high: first digit is productivity, second is shade."}
  par( mfrow=c(3,3), mar=c(4,4,3,1) )
...
```{r XX32sim, fig.height=5.5, fig.cap="\\label{fig:XX32sim}Simulations for 4 farm types in Guatemala. Farm type codes ranging from 1 = very low to 4 = high: first digit is productivity, second is shade."}
  par( mfrow=c(3,4), mar=c(4,4,3,1) )
...
```{r XX32F1234, fig.height=7.5, fig.cap="\\label{fig:XX32F1234}Four different simulations (blue) for 4 farm types in Guatemala. Farm type codes ranging from 1 = very low to 4 = high: first digit is productivity, second is shade."}
  par( mfrow=c(5,4), mar=c(1,2,1,1) )

  ysim    <- "Shade_fb"
  ysim1.13 <- oF1_G[ if_G.type13, ysim ] ; ysim1.24 <- oF1_G[ if_G.type24, ysim ]
  ysim1.33 <- oF1_G[ if_G.type33, ysim ] ; ysim1.43 <- oF1_G[ if_G.type43, ysim ]
  ysim2.13 <- oF2_G[ if_G.type13, ysim ] ; ysim2.24 <- oF2_G[ if_G.type24, ysim ]
  ysim2.33 <- oF2_G[ if_G.type33, ysim ] ; ysim2.43 <- oF2_G[ if_G.type43, ysim ]
  ysim3.13 <- oF3_G[ if_G.type13, ysim ] ; ysim3.24 <- oF3_G[ if_G.type24, ysim ]
  ysim3.33 <- oF3_G[ if_G.type33, ysim ] ; ysim3.43 <- oF3_G[ if_G.type43, ysim ]
  ysim4.13 <- oF4_G[ if_G.type13, ysim ] ; ysim4.24 <- oF4_G[ if_G.type24, ysim ]
  ysim4.33 <- oF4_G[ if_G.type33, ysim ] ; ysim4.43 <- oF4_G[ if_G.type43, ysim ]
  ysim1.   <- list( ysim1.13, ysim1.24, ysim1.33, ysim1.43 )
  ysim2.   <- list( ysim2.13, ysim2.24, ysim2.33, ysim2.43 )
  ysim3.   <- list( ysim3.13, ysim3.24, ysim3.33, ysim3.43 )
  ysim4.   <- list( ysim4.13, ysim4.24, ysim4.33, ysim4.43 )
  boxplot( ysim1., names=rep("",4), main="Shade 1", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim2., names=rep("",4), main="Shade 2", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  ...
  ysim3.   <- list( ysim3.13, ysim3.24, ysim3.33, ysim3.43 )
  ysim4.   <- list( ysim4.13, ysim4.24, ysim4.33, ysim4.43 )
  boxplot( ysim1., names=c(1.3, 2.4, 3.3, 4.3), main="C-runoff 1", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,600))
  boxplot( ysim2., names=c(1.3, 2.4, 3.3, 4.3), main="C-runoff 2", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,600))
  boxplot( ysim3., names=c(1.3, 2.4, 3.3, 4.3), main="C-runoff 3", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,600))
  boxplot( ysim4., names=c(1.3, 2.4, 3.3, 4.3), main="C-runoff 4", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,600))
```

```{r XX32F1234diff, fig.height=7.5, fig.cap="\\label{fig:XX32F1234fiff}Four different simulations (blue) for 4 farm types in Guatemala. Farm type codes ranging from 1 = very low to 4 = high: first digit is productivity, second is shade."}
  par( mfrow=c(5,4), mar=c(1,2,1,1) )
  ...
  ysim4.13 <- oF4_G[ if_G.type13, ysim ] ; ysim4.24 <- oF4_G[ if_G.type24, ysim ]
  ysim4.33 <- oF4_G[ if_G.type33, ysim ] ; ysim4.43 <- oF4_G[ if_G.type43, ysim ]
  ysim1.   <- list( ysim1.13         , ysim1.24         ,
                    ysim1.33         , ysim1.43 )
  ysim2.   <- list( ysim2.13-ysim1.13, ysim2.24-ysim1.24,
                    ysim2.33-ysim1.33, ysim2.43-ysim1.43 )
  ysim3.   <- list( ysim3.13-ysim1.13, ysim3.24-ysim1.24,
                    ysim3.33-ysim1.33, ysim3.43-ysim1.43 )
  ysim4.   <- list( ysim4.13-ysim1.13, ysim4.24-ysim1.24,
                    ysim4.33-ysim1.33, ysim4.43-ysim1.43 )
  boxplot( ysim1., names=rep("",4), main="Shade 1", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim2., names=rep("",4), main="Shade 2", col="blue",
           cex.main=0.7, cex.axis=0.7, ylim=c(-0.6,0.6))
  ...
  boxplot( ysim1., names=c(1.3, 2.4, 3.3, 4.3), main="C-runoff 1", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,600))
  boxplot( ysim2., names=c(1.3, 2.4, 3.3, 4.3), main="C-runoff 2", col="blue",
           cex.main=0.7, cex.axis=0.7, ylim=c(-300,300))
  boxplot( ysim3., names=c(1.3, 2.4, 3.3, 4.3), main="C-runoff 3", col="blue",
           cex.main=0.7, cex.axis=0.7, ylim=c(-300,300))
  boxplot( ysim4., names=c(1.3, 2.4, 3.3, 4.3), main="C-runoff 4", col="blue",
           cex.main=0.7, cex.axis=0.7, ylim=c(-300,300))
```

```{r, eval=T, fig.height=8.5}
  par( mfrow=c(6,4), mar=c(1,2,1,1) )

  ysim    <- "Nmineralis_fb"
  ysim1.12 <- oF1_C[ if_C.type12, ysim ]
  ysim1.24 <- oF1_C[ if_C.type24, ysim ]
  ysim1.32 <- oF1_C[ if_C.type32, ysim ]
  ysim1.43 <- oF1_C[ if_C.type43, ysim ]
  ysim2.12 <- oF2_C[ if_C.type12, ysim ]
  ysim2.24 <- oF2_C[ if_C.type24, ysim ]
  ysim2.32 <- oF2_C[ if_C.type32, ysim ]
  ysim2.43 <- oF2_C[ if_C.type43, ysim ]
  ysim3.12 <- oF3_C[ if_C.type12, ysim ]
  ysim3.24 <- oF3_C[ if_C.type24, ysim ]
  ysim3.32 <- oF3_C[ if_C.type32, ysim ]
  ysim3.43 <- oF3_C[ if_C.type43, ysim ]
  ysim4.12 <- oF4_C[ if_C.type12, ysim ]
  ysim4.24 <- oF4_C[ if_C.type24, ysim ]
  ysim4.32 <- oF4_C[ if_C.type32, ysim ]
  ysim4.43 <- oF4_C[ if_C.type43, ysim ]
  ysim1.   <- list( ysim1.12, ysim1.24, ysim1.32, ysim1.43 )
  ysim2.   <- list( ysim2.12, ysim2.24, ysim2.32, ysim2.43 )
  ysim3.   <- list( ysim3.12, ysim3.24, ysim3.32, ysim3.43 )
  ysim4.   <- list( ysim4.12, ysim4.24, ysim4.32, ysim4.43 )
  boxplot( ysim1., names=rep("",4), main="N-mineralis. 1", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim2., names=rep("",4), main="N-mineralis. 2", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim3., names=rep("",4), main="N-mineralis. 3", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim4., names=rep("",4), main="N-mineralis. 4", col="cyan",
           cex.main=0.7, cex.axis=0.7)
  
  ysim    <- "Nmineralis_fb"
  ysim1.13 <- oF1_G[ if_G.type13, ysim ]
  ysim1.24 <- oF1_G[ if_G.type24, ysim ]
  ysim1.33 <- oF1_G[ if_G.type33, ysim ]
  ysim1.43 <- oF1_G[ if_G.type43, ysim ]
  ysim2.13 <- oF2_G[ if_G.type13, ysim ]
  ysim2.24 <- oF2_G[ if_G.type24, ysim ]
  ysim2.33 <- oF2_G[ if_G.type33, ysim ]
  ysim2.43 <- oF2_G[ if_G.type43, ysim ]
  ysim3.13 <- oF3_G[ if_G.type13, ysim ]
  ysim3.24 <- oF3_G[ if_G.type24, ysim ]
  ysim3.33 <- oF3_G[ if_G.type33, ysim ]
  ysim3.43 <- oF3_G[ if_G.type43, ysim ]
  ysim4.13 <- oF4_G[ if_G.type13, ysim ]
  ysim4.24 <- oF4_G[ if_G.type24, ysim ]
  ysim4.33 <- oF4_G[ if_G.type33, ysim ]
  ysim4.43 <- oF4_G[ if_G.type43, ysim ]
  ysim1.   <- list( ysim1.13, ysim1.24, ysim1.33, ysim1.43 )
  ysim2.   <- list( ysim2.13, ysim2.24, ysim2.33, ysim2.43 )
  ysim3.   <- list( ysim3.13, ysim3.24, ysim3.33, ysim3.43 )
  ysim4.   <- list( ysim4.13, ysim4.24, ysim4.33, ysim4.43 )
  boxplot( ysim1., names=rep("",4), main="N-mineralis. 1", col="orange",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim2., names=rep("",4), main="N-mineralis. 2", col="orange",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim3., names=rep("",4), main="N-mineralis. 3", col="orange",
           cex.main=0.7, cex.axis=0.7)
  boxplot( ysim4., names=rep("",4), main="N-mineralis. 4", col="orange",
           cex.main=0.7, cex.axis=0.7)

  ysim    <- "Nleaching_fb"
  x       <- df_C.f[,"Nfert"] ; x[ x==0 ] <- NA
  ysim1.12 <- oF1_C[ if_C.type12, ysim ] / x[if_C.type12]
  ysim1.24 <- oF1_C[ if_C.type24, ysim ] / x[if_C.type24]
  ysim1.32 <- oF1_C[ if_C.type32, ysim ] / x[if_C.type32]
  ysim1.43 <- oF1_C[ if_C.type43, ysim ] / x[if_C.type43]
  ysim2.12 <- oF2_C[ if_C.type12, ysim ] / x[if_C.type12]
  ysim2.24 <- oF2_C[ if_C.type24, ysim ] / x[if_C.type24]
  ysim2.32 <- oF2_C[ if_C.type32, ysim ] / x[if_C.type32]
  ysim2.43 <- oF2_C[ if_C.type43, ysim ] / x[if_C.type43]
  ysim3.12 <- oF3_C[ if_C.type12, ysim ] / x[if_C.type12]
  ysim3.24 <- oF3_C[ if_C.type24, ysim ] / x[if_C.type24]
  ysim3.32 <- oF3_C[ if_C.type32, ysim ] / x[if_C.type32]
  ysim3.43 <- oF3_C[ if_C.type43, ysim ] / x[if_C.type43]
  ysim4.12 <- oF4_C[ if_C.type12, ysim ] / x[if_C.type12]
  ysim4.24 <- oF4_C[ if_C.type24, ysim ] / x[if_C.type24]
  ysim4.32 <- oF4_C[ if_C.type32, ysim ] / x[if_C.type32]
  ysim4.43 <- oF4_C[ if_C.type43, ysim ] / x[if_C.type43]
  ysim1.   <- list( ysim1.12, ysim1.24, ysim1.32, ysim1.43 )
  ysim2.   <- list( ysim2.12, ysim2.24, ysim2.32, ysim2.43 )
  ysim3.   <- list( ysim3.12, ysim3.24, ysim3.32, ysim3.43 )
  ysim4.   <- list( ysim4.12, ysim4.24, ysim4.32, ysim4.43 )
  boxplot( ysim1., names=rep("",4), main="N-leaching 1", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,1))
  boxplot( ysim2., names=rep("",4), main="N-leaching 2", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,1))
  boxplot( ysim3., names=rep("",4), main="N-leaching 3", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,1))
  ...
  ysim    <- "Nleaching_fb"
  x       <- df_G.f[,"Nfert"] ; x[ x==0 ] <- NA
  ysim1.13 <- oF1_G[ if_G.type13, ysim ] / x[if_G.type13]
  ysim1.24 <- oF1_G[ if_G.type24, ysim ] / x[if_G.type24]
  ysim1.33 <- oF1_G[ if_G.type33, ysim ] / x[if_G.type33]
  ysim1.43 <- oF1_G[ if_G.type43, ysim ] / x[if_G.type43]
  ysim2.13 <- oF2_G[ if_G.type13, ysim ] / x[if_G.type13]
  ysim2.24 <- oF2_G[ if_G.type24, ysim ] / x[if_G.type24]
  ysim2.33 <- oF2_G[ if_G.type33, ysim ] / x[if_G.type33]
  ysim2.43 <- oF2_G[ if_G.type43, ysim ] / x[if_G.type43]
  ysim3.13 <- oF3_G[ if_G.type13, ysim ] / x[if_G.type13]
  ysim3.24 <- oF3_G[ if_G.type24, ysim ] / x[if_G.type24]
  ysim3.33 <- oF3_G[ if_G.type33, ysim ] / x[if_G.type33]
  ysim3.43 <- oF3_G[ if_G.type43, ysim ] / x[if_G.type43]
  ysim4.13 <- oF4_G[ if_G.type13, ysim ] / x[if_G.type13]
  ysim4.24 <- oF4_G[ if_G.type24, ysim ] / x[if_G.type24]
  ysim4.33 <- oF4_G[ if_G.type33, ysim ] / x[if_G.type33]
  ysim4.43 <- oF4_G[ if_G.type43, ysim ] / x[if_G.type43]
  ysim1.   <- list( ysim1.13, ysim1.24, ysim1.33, ysim1.43 )
  ysim2.   <- list( ysim2.13, ysim2.24, ysim2.33, ysim2.43 )
  ysim3.   <- list( ysim3.13, ysim3.24, ysim3.33, ysim3.43 )
  ysim4.   <- list( ysim4.13, ysim4.24, ysim4.33, ysim4.43 )
  boxplot( ysim1., names=rep("",4), main="N-leaching 1", col="orange",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,1))
  boxplot( ysim2., names=rep("",4), main="N-leaching 2", col="orange",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,1))
  boxplot( ysim3., names=rep("",4), main="N-leaching 3", col="orange",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,1))
  boxplot( ysim4., names=rep("",4), main="N-leaching 4", col="orange",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,1))
  ...
  ysim3.   <- list( ysim3.12, ysim3.24, ysim3.32, ysim3.43 )
  ysim4.   <- list( ysim4.12, ysim4.24, ysim4.32, ysim4.43 )
  boxplot( ysim1., names=rep("",4), main="N-emission 1", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,1))
  boxplot( ysim2., names=rep("",4), main="N-emission 2", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,1))
  boxplot( ysim3., names=rep("",4), main="N-emission 3", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,1))
  boxplot( ysim4., names=rep("",4), main="N-emission 4", col="cyan",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,1))
  ...
  ysim3.   <- list( ysim3.13, ysim3.24, ysim3.33, ysim3.43 )
  ysim4.   <- list( ysim4.13, ysim4.24, ysim4.33, ysim4.43 )
  boxplot( ysim1., names=rep("",4), main="N-emission 1", col="orange",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,1))
  boxplot( ysim2., names=rep("",4), main="N-emission 2", col="orange",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,1))
  boxplot( ysim3., names=rep("",4), main="N-emission 3", col="orange",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,1))
  boxplot( ysim4., names=rep("",4), main="N-emission 4", col="orange",
           cex.main=0.7, cex.axis=0.7, ylim=c(0,1))
```
...
```{r}
  iF_short <- function( df=df_C.f ) {
    n.f <- dim(df)[1]
    iF_short <- cbind.data.frame(
      "code"           = df$code,
      "type"           = df$type,
      "lat"            = round( df$lat, 2 ),
      "lon"            = round( df$lon, 2 ),
      "alt"            = df$altitude,
      "slope"          = round( df$slope, 1 ),
      "DM.soil"        = round( df$DM.soil, 2 ),
      "C.soil"         = round( df$C.soil, 2 ),
      "N.soil"         = round( df$N.soil, 2 ),
      "WP"             = round( df$WP, 2 ),
      "FC"             = round( df$FC, 2 ),
      "GR"             = sapply( 1:n.f, function(i){
        round( mean(df$weather.WC[[i]][,"GR"]), 1 ) } ),
      "TMIN"           = sapply( 1:n.f, function(i){
        round( mean(df$weather.WC[[i]][,"TMIN"]), 1 ) } ),
      "TMAX"           = sapply( 1:n.f, function(i){
        round( mean(df$weather.WC[[i]][,"TMAX"]), 1 ) } ),
      "VP"             = sapply( 1:n.f, function(i){
        round( mean(df$weather.WC[[i]][,"VP"]), 2 ) } ),
      "WN"             = sapply( 1:n.f, function(i){
        round( mean(df$weather.WC[[i]][,"WN"]), 1 ) } ),
      "RAIN"           = sapply( 1:n.f, function(i){
        round( 365 * mean(df$weather.WC[[i]][,"RAIN"]), 0 ) } ),
      "shade"          = round( df$shade, 2 ),
      "dens_Erythrina" = df$dens_Erythrina,
      "dens_Inga"      = df$dens_Inga,
      "dens_Banana"    = df$dens_Banana,
      "dens_Avocado"   = df$dens_Avocado,
      "dens_Grevillea" = df$dens_Grevillea,
      "dens_Cordia"    = df$dens_Cordia,
      "yield1819"      = round( df$yield1819, 0 ),
      "yield1920"      = round( df$yield1920, 0 )
      )
    return( iF_short)
  }
```
...
```{r}
  oF_short <- function( oF=oF1_C, df=df_C.f ) {
    oF_short <- cbind(
      "Simulation"      = oF[,"FarmSetting" ],
      "code"            = df$code,
      "Shade_fb"        = round( oF[,"Shade_fb"       ], 2 ),
      "harvDM_f_hay"    = round( oF[,"harvDM_f_hay"   ], 0 ),
      "sd.harvDM_f_hay" = round( oF[,"sd.harvDM_f_hay"], 0 ),
      "harvDM_f_ha1819" = round( oF[,"harvDM_f_ha1819"], 0 ),
      "harvDM_f_ha1920" = round( oF[,"harvDM_f_ha1920"], 0 ),
      "harvDMPT_f_hay"  = round( oF[,"harvDMPT_f_hay" ], 0 ),
      "harvDMST_3_hay"  = round( oF[,"harvDMST_3_hay" ], 0 ),
      "D_Csoil"         = round( oF[,"D_Csoil"        ], 0 ),
      "Crunoff_fb"      = round( oF[,"Crunoff_fb"     ], 0 ),
      "Nmineralis_fb"   = round( oF[,"Nmineralis_fb"  ], 0 ),
      "NfixT_fb"        = round( oF[,"NfixT_fb"       ], 0 ),
      "Nleaching_fb"    = round( oF[,"Nleaching_fb"   ], 0 ),
      "Nemission_fb"    = round( oF[,"Nemission_fb"   ], 0 ) )
    return( oF_short)
  }
```

```{r}
  var.o <- c( "code", "type", "lat", "lon", "alt", "slope",
    "DM.soil", "C.soil", "N.soil", "WP", "FC",
    "GR", "TMIN", "TMAX", "VP", "WN", "RAIN",
    "shade", "dens_Erythrina", "dens_Inga", "dens_Banana",
    "dens_Avocado", "dens_Grevillea", "dens_Cordia",
    "yield1819", "yield1920" )
  unit.o    <- c( "-", "-", "deg", "deg", "m", "deg",
    "g DM cm-3", "kg C m-2", "kg N m-2", "m3 m-3", "m3 m-3",
    "MJ m-2 d-1", "degC", "degC", "kPa", "m s-1", "mm y-1",
    "m2 m-2", "ha-1", "ha-1", "ha-1",
    "ha-1", "ha-1", "ha-1",
    "kg DM ha-1 y-1", "kg DM ha-1 y-1" )
  meaning.o <-c( "Farm code (first digit: 1=Costa Rica, 2=Guatemala)",
    "Farm type for productivity and shade (1=Very low, 2=Low, 3=Medium, 4=High)",
    "Latitude",
    "Longitude",
    "Altitude",
    "Soil slope",
    ...
    "Soil wilting point",
    "Soil field Capacity",
    "Global radiation 2005-2020",
    "Daily minimum temperature 2005-2020",
    "Daily maximum temperature 2005-2020",
    "Vapour pressure 2005-2020",
    "Wind speed 2005-2020",
    "Precipitation 2005-2020",
    "Shade",
    "Initial planting density Erythrina",
    "Initial planting density Inga",
    "Initial planting density Banana",
    "Initial planting density Avocado",
    "Initial planting density Grevillea",
    "Initial planting density Cordia",
    "Coffee dry bean yield in 2018-19",
    "Coffee dry bean yield in 2019-20" )
```

```{r}
  var.s  <- c( "Simulation", "code",
      "Shade_fb",
      "harvDM_f_hay", "sd.harvDM_f_hay", "harvDM_f_ha1819", "harvDM_f_ha1920",
      "harvDMPT_f_hay", "harvDMST_3_hay",
      "D_Csoil", "Crunoff_fb",
      "Nmineralis_fb", "NfixT_fb", "Nleaching_fb", "Nemission_fb" )
  unit.s    <- c( "-",
      "-",
      "m2 m-2",
      "kg DM ha-1 y-1",
      ...
      "kg N ha-1 y-1",
      "kg N ha-1 y-1" )
  meaning.s <- c( "Simulation: 1=Default settings, 2=N-fertilisation 50%, 3=pruning intensity increased (shade target 10%), 4=pruning intensity increased (shade target 10%) & tree planting density 20%",
      "Farm code (first digit: 1=Costa Rica, 2=Guatemala)",
      "Shade in final two years",
      "Coffee dry bean yield 2005-20: Average",
      "Coffee dry bean yield 2005-20: Standard deviation",
      "Coffee dry bean yield in harvest season 2018-19",
      "Coffee dry bean yield in harvest season 2019-20",
      "Fruit production 2005-2020",
      "Timber production 2005-2020",
      "Soil C change 2005-2020",
      "C runoff in final two years",
      "N-mineralisation in final two years",
      "N-fixation in final two years",
      "N-leaching in final two years",
      "N-emission in final two years (sum of N2O, NO and N2)" )
```

```{r}
  dictionary <- rbind(
    c("SEACAF farms in Costa Rica and Guatemala: CAF2021 input and output","",""),
    c("Marcel van Oijen","",as.character(Sys.Date())),
    c("", "", ""),
    c("OBSERVATIONS (2019 & 2020)", "", ""),
    cbind( var.o, unit.o, meaning.o ),
    c("", "", ""),
    c("SIMULATIONS (Aug 2005 - Jul 2020)", "", ""),
    cbind( var.s, unit.s, meaning.s ),
    c("", "", ""),
    c("WORKSHEETS","",""),
    c("C1:C4","Costa Rica simulations 1:4","1=Default settings, 2=N-fertilisation 50%, 3=pruning intensity increased (shade target 10%), 4=pruning intensity increased (shade target 10%) & tree planting density 20%"),
    c("G1:G4","Guatemala simulations 1:4","1=Default settings, 2=N-fertilisation 50%, 3=pruning intensity increased (shade target 10%), 4=pruning intensity increased  (shade target 10%) & tree planting density 20%")
  )
```

```{r}
  IOlist <- list( "DICTIONARY"        = data.frame(dictionary),
                  "Costa Rica (obs.)" = data.frame(iF_short(df_C.f),stringsAsFactors=T),
                  "Guatemala (obs.)"  = data.frame(iF_short(df_G.f),stringsAsFactors=T),
                  "C1" = data.frame( oF_short(oF1_C,df_C.f)),
                  "G1" = data.frame( oF_short(oF1_G,df_G.f)),
                  "C2" = data.frame( oF_short(oF2_C,df_C.f)),
                  "G2" = data.frame( oF_short(oF2_G,df_G.f)),
                  "C3" = data.frame( oF_short(oF3_C,df_C.f)),
                  "G3" = data.frame( oF_short(oF3_G,df_G.f)),
                  "C4" = data.frame( oF_short(oF4_C,df_C.f)),
                  "G4" = data.frame( oF_short(oF4_G,df_G.f)) )

  writexl::write_xlsx( IOlist,
                       "CAF2021_input_output_farms.xlsx" )
  
  # openxlsx::write.xlsx( outlist,
  #             "CAF2021_output_farms.xlsx", 
  #             col.names=T, row.names=T,
  #             append=T, overwrite=T )
```
