...
```{r Balances, fig.height=6, fig.cap="\\label{fig:Balances}Biogeochemical balances for nitrogen, carbon and water at Masatepe in the ILSS_CI system (shading by both Inga laurina and Samanea saman; Fertilisation-mode CI). Left bar from each pair: gains; right bar: losses. All rates are averages for the whole period of simulation."}
  ...
  plot_NbalanceSoil  (output, title1="Soil N-balance"  , title2="", ver=T)
  par( mar=c(3,3,3,0) )
  plot_CbalanceSoil  (output, title1="Soil C-balance"  , title2="", ver=T)
  par( mar=c(3,3,3,0) )
  plot_H2ObalanceSoil(output, title1="Soil H2O-balance", title2="", ver=T)
  par( mar=c(3,3,3,0) )
  plot_CbalanceSystem(output, title1="System C-balance", title2="", ver=T)
```

Comparison of leaving tree branches on the field after thinning and pruning (default) vs. removing them as harvest (customary in NIC and GTM).

```{r}
  source("initialisation/initialise_CAF2021_Masatepe_09_ILSS_CI.R")
  output <- run_model()
  source("initialisation/initialise_CAF2021_Masatepe_09_ILSS_CI.R")
  params <- set_par( c("FHARVBT(1)","FHARVBT(3)","FHARVBT(3)"),
                     c( 1          , 1          , 1          ) )
  output2 <- run_model()
  ...
  plot_CbalanceSoil  (output, title1="Soil C-balance"  , title2="", ver=T)
  par( mar=c(3,3,3,0) )
  plot_NbalanceSoil  (output2, title1="Soil N-balance 2"  , title2="", ver=T)
  par( mar=c(3,3,3,0) )
  plot_CbalanceSoil  (output2, title1="Soil C-balance 2"  , title2="", ver=T)
```
...
```{r Nbalance4Nlevels, fig.height=6, fig.cap="\\label{fig:Nbalance4Nlevels}Soil nitrogen balances for 4 different treatments at Masatepe in the ILSS system (shading by both Inga laurina and Samanea saman). Gains on the left, losses on the right."}
  ...
  source("initialisation/initialise_CAF2021_Masatepe_09_ILSS_CI.R")
  output <- run_model()
  plot_NbalanceSoil(output, title1="", title2="CI", ymax=800, ver=T)
  par(mar=c(3,3,3,0))
  source("initialisation/initialise_CAF2021_Masatepe_10_ILSS_CM.R")
  output <- run_model()
  plot_NbalanceSoil(output, title1="", title2="CM", ymax=800, ver=T)
  par(mar=c(3,3,3,0))
  source("initialisation/initialise_CAF2021_Masatepe_11_ILSS_OI.R")
  output <- run_model()
  plot_NbalanceSoil(output, title1="", title2="OI", ymax=800, ver=T)
  par(mar=c(3,3,3,0))
  source("initialisation/initialise_CAF2021_Masatepe_12_ILSS_OM.R")
  output <- run_model()
  plot_NbalanceSoil(output, title1="", title2="OM", ymax=800, ver=T)
```
...
# Bayesian calibration for Masatepe
...
```{r, eval=F, out.width="100%", fig.cap="\\label{fig:YieldsObsSim}Simulated vs. observed annual yields for the 14 treatments in Masatepe 2005-2014. Left: all 140 recorded and simulated yields. Right: average yield for each of the treatments."}
  load('BC_00_10.RData')
