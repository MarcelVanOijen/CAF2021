...
```{r}
  plot_NbalanceSoil <- function( sim=output, title1="", title2=title1,
    ...
    barplot( as.matrix(NinMean),
             ...
             legend.text=leg,
             args.legend=list( title="Inputs (kg ha-1 y-1)", cex=0.9,
    ...
    barplot( as.matrix(NoutMean),
             ...
             beside=F, ylim=c(0,y.max), col=1:length(leg),
             legend.text=leg,
             args.legend=list( title="Outputs (kg ha-1 y-1)", cex=0.9,
                               x="bottom", bg="white", inset=c(-0.3,0) ) )
    title(title2,adj=0)
    if(verify) {
...
```{r}
  plot_H2ObalanceSoil <- function( sim=output, title1="", title2=title1,
    ...
    barplot( as.matrix(WinMean),
             beside=F, ylim=c(0,y.max), col=1:length(leg),
             legend.text=leg,
             args.legend=list( title="Inputs (mm d-1)", cex=0.9,
                               x="bottom", bg="white", inset=c(-0.3,0) ) )
    title(title1,adj=1)
    par(mar=c(3,0,3,3))
    leg <- WoutNames
    barplot( as.matrix(WoutMean),
             ...
             beside=F, ylim=c(0,y.max), col=1:length(leg),
             legend.text=leg,
             args.legend=list( title="Outputs (mm d-1)", cex=0.9,
                               x="bottom", bg="white", inset=c(-0.3,0) ) )
    title(title2,adj=0)
    if(verify) {
...
```{r OutputsCarbonCoffee, echo=F, fig.height=5, fig.cap="\\label{fig:OutputsLCC}Cumulative time series for the carbon in coffee"}
  source("initialisation/initialise_CAF2021_Turrialba_01_E_AC.R")
  # source("initialisation/initialise_CAF2021_Turrialba_04_E_BO.R")
