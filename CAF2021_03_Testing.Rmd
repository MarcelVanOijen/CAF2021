...
```{r}
  ...
  barplot( as.matrix(NinMean),
         ...
         legend.text=leg, args.legend=list( x="right", inset=c(-0.3,0) ) )
...
Verify the N-balance.

```{r}
  Nsoil_f_hay   <- output[ , which( outputNames=="Nsoil_f") ] * 1.e3 # kgN ha-1
  D_Nsoil_f_hay <- tail(Nsoil_f_hay,1) - head(Nsoil_f_hay,1)         # kgN ha-1
  ...
  cat( "VERIFYING THE N-BALANCE FOR THE SOIL (kgN ha-1).",
        "\nState variable change: ", D_Nsoil_f_hay,
