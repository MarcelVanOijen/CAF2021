...
```{r}
  Nsoil_f_kgha   <- output[ , which( outputNames=="Nsoil_f") ] * 1.e3 # kgN ha-1
  D_Nsoil_f_kgha <- tail(Nsoil_f_kgha,1) - head(Nsoil_f_kgha,1)       # kgN ha-1
  
  NinSum         <- colSums(output[2:NDAYS,i.Nin ])            * 1.e4 # kgN ha-1
  NoutSum        <- colSums(output[2:NDAYS,i.Nout])            * 1.e4 # kgN ha-1
  NinMinusNout   <- sum(NinSum) - sum(NoutSum)
  
  cat( "VERIFYING THE N-BALANCE FOR THE SOIL (kgN ha-1):",
       "\nState variable change: ", D_Nsoil_f_kgha,
...
```{r}
  Csoil_f_kgha   <- output[ , which( outputNames=="Csoil_f") ] * 1.e3 # kgC ha-1
  D_Csoil_f_kgha <- tail(Csoil_f_kgha,1) - head(Csoil_f_kgha,1)       # kgC ha-1
  
  CinSum        <- colSums(output[2:NDAYS,i.Cin ])             * 1.e4 # kgC ha-1
  CoutSum       <- colSums(output[2:NDAYS,i.Cout])             * 1.e4 # kgC ha-1
  CinMinusCout  <- sum(CinSum) - sum(CoutSum)
  
  cat( "VERIFYING THE C-BALANCE FOR THE SOIL (kgC ha-1):",
       "\nState variable change: ", D_Csoil_f_kgha,
...
```{r}
  ...
  D_WA_f <- tail(WA_f,1) - head(WA_f,1)             # mm
...
```{r}
  ...
  CoutNames <- c("Rsoil_f", "Crunoff_f", "harvC_f", "harvCST_f")
  nCin      <- length(CinNames) ; nCout  <- length(CoutNames)
  i.Cin     <- rep(NA,nCin)     ; i.Cout <- rep(NA,nCout)
  for(i in 1:nCin ) {i.Cin [i] <- which( outputNames==CinNames [i] ) }
  for(i in 1:nCout) {i.Cout[i] <- which( outputNames==CoutNames[i] ) }
...
```{r}
  C_f_kgha      <- output[ , which( outputNames=="C_f" ) ] * 1.e3 # kgC ha-1
  CT_f_kgha     <- output[ , which( outputNames=="CT_f") ] * 1.e3 # kgC ha-1
  Csys_f_kgha   <- C_f_kgha + CT_f_kgha + Csoil_f_kgha            # kgC ha-1
  D_Csys_f_kgha <- Csys_f_kgha[NDAYS] - Csys_f_kgha[1]            # kgC ha-1
  # D_Csys_f_kgha <- Csys_f_kgha[1635] - Csys_f_kgha[1]            # kgC ha-1
  
  CinSum       <- colSums(output[2:NDAYS,i.Cin ])          * 1.e4 # kgC ha-1
  CoutSum      <- colSums(output[2:NDAYS,i.Cout])          * 1.e4 # kgC ha-1
  # CinSum       <- colSums(output[2:1635,i.Cin ])          * 1.e4 # kgC ha-1
  # CoutSum      <- colSums(output[2:1635,i.Cout])          * 1.e4 # kgC ha-1
  CinMinusCout <- sum(CinSum) - sum(CoutSum)
  
  cat( "VERIFYING THE C-BALANCE FOR THE SYSTEM (kgC ha-1):",
       "\nState variable change: ", D_Csys_f_kgha,
       "\nInputs minus outputs : ", CinMinusCout )
```
