  ...
  plot_CdistributionTrees <- function( sim=output, it=1:3 ) {
    ...
    for(t in it) {
      ...
      plot( Time, CRT+CST+CBT+CLT+CPT, ylim=c(0,y.max),
            main=paste0("Carbon in tree species ",t," (kg C m-2)"),
            xlab="", ylab="", type="l" )
...
```{r}
  ...
  params[names_params=="TREEDENS0(2)"] <- 0.04
  params[names_params=="CBtree0"     ] <- 1 # 0.003
  params[names_params=="CLtree0"     ] <- 1 # 0.0003
  params[names_params=="CRtree0"     ] <- 1 # 0.03
  params[names_params=="CStree0"     ] <- 1 # 0.013
  params[names_params=="FWT(2)"      ] <- 0.5
  params[names_params=="FST(2)"      ] <- 0.5
  params[names_params=="FPT(2)"      ] <- 0.5
  params[names_params=="TBEFOREPT(2)"] <- 100
  params[names_params=="TCBT(2)"     ] <- 365
  params[names_params=="TCRT(2)"     ] <- 365
  params[names_params=="TCST(2)"     ] <- 365
  params[names_params=="KH(2)"       ] <- 4    # 6
  # params[names_params=="KHEXP(2)"    ] <- 0.35 # 0.35
  params[names_params=="KAC(2)"      ] <- 6   # 30
  # params[names_params=="KACEXP(2)"   ] <- 0.5  # 0.5
  output <- run_model()
  ...
  plot( Time, h_t2     , type="l", ylab="", main="Tree height (m)" )
  plot( Time, CAtree2  , type="l", ylab="", main="Crown area (m2 tree-1)" )
  plot( Time, 1-Ac1    , type="l", ylab="", main="Shade (-)" )
  plot( Time, YcoffeeDM, type="l", ylab="", main="Yield coffee (kg DM ha-1)" )
  plot( Time, YtreesDM , type="l", ylab="", main="Yield trees  (kg DM ha-1)" )
```
