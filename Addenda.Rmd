  ...
  for(fi in 1:n_C.f) {
    ...
    if((!is.na(shade))) {
      cat( paste0("Shade_f\t2019\t300\t"     ,shade,"\t",0.50,"\n"),
           file=file.f, append=TRUE ) }
    if(fi %in% if_C.1E) {
      cat( paste0("h_t(1)\t2020\t139\t"      ,  8  ,"\t",1.00,"\n"),
           file=file.f, append=TRUE )
      cat( paste0("CAtree_t(1)\t2020\t139\t" , 25  ,"\t",1.00,"\n"),
           file=file.f, append=TRUE ) }
    if(fi %in% if_C.1I) {
      cat( paste0("h_t(1)\t2020\t139\t"      ,  8  ,"\t",1.00,"\n"),
           file=file.f, append=TRUE )
      cat( paste0("CAtree_t(1)\t2020\t139\t" , 50  ,"\t",1.00,"\n"),
           file=file.f, append=TRUE ) }
    if(fi %in% if_C.2B) {
      cat( paste0("h_t(2)\t2020\t139\t"      ,  8  ,"\t",1.00,"\n"),
           file=file.f, append=TRUE )
      cat( paste0("CAtree_t(2)\t2020\t139\t" , 25  ,"\t",1.00,"\n"),
           file=file.f, append=TRUE ) }
    if(fi %in% if_C.2A) {
      cat( paste0("h_t(2)\t2020\t139\t"      ,  8  ,"\t",1.00,"\n"),
           file=file.f, append=TRUE )
      cat( paste0("CAtree_t(2)\t2020\t139\t" , 25  ,"\t",1.00,"\n"),
           file=file.f, append=TRUE ) }
    if(fi %in% if_C.3G) {
      cat( paste0("h_t(3)\t2020\t139\t"      , 15  ,"\t",1.00,"\n"),
           file=file.f, append=TRUE )
      cat( paste0("CAtree_t(3)\t2020\t139\t" ,100  ,"\t",1.00,"\n"),
           file=file.f, append=TRUE ) }
    if(fi %in% if_C.3C) {
      cat( paste0("h_t(3)\t2020\t139\t"      , 30  ,"\t",1.00,"\n"),
           file=file.f, append=TRUE )
      cat( paste0("CAtree_t(3)\t2020\t139\t" ,200  ,"\t",1.00,"\n"),
           file=file.f, append=TRUE ) }
  }
```
...
```{r list_outF_C}
  ...
  for(fi in 1:n_C.f) {
    ...
    for(iF in 1:nF_C) {
      ...
      if( iF==3 ) {
        params <- matMAP_C[,fi]
        ipAC         <- which( startsWith(names_params,"KAC") )
        ipH          <- which( startsWith(names_params,"KH" ) )
        params[ipAC] <- 1.2 * params[ipAC]
        params[ipH ] <- 1.2 * params[ipH ]
...
```{r XX31, fig.height=7.5, fig.cap="\\label{fig:XX31}XX31."}
  ...
  yp <- "KH(1)"     ; plotimpacts_CG.Nfert( FC=pF2_C, FG=pF2_G, yvar=yp )
  yp <- "KHEXP(1)"  ; plotimpacts_CG.Nfert( FC=pF2_C, FG=pF2_G, yvar=yp )
  yp <- "KAC(1)"    ; plotimpacts_CG.Nfert( FC=pF2_C, FG=pF2_G, yvar=yp )
  yp <- "KACEXP(1)" ; plotimpacts_CG.Nfert( FC=pF2_C, FG=pF2_G, yvar=yp )
  
  yp <- "KH(2)"     ; plotimpacts_CG.Nfert( FC=pF2_C, FG=pF2_G, yvar=yp )
  yp <- "KHEXP(2)"  ; plotimpacts_CG.Nfert( FC=pF2_C, FG=pF2_G, yvar=yp )
  yp <- "KAC(2)"    ; plotimpacts_CG.Nfert( FC=pF2_C, FG=pF2_G, yvar=yp )
  yp <- "KACEXP(2)" ; plotimpacts_CG.Nfert( FC=pF2_C, FG=pF2_G, yvar=yp )

  yp <- "KH(3)"     ; plotimpacts_CG.Nfert( FC=pF2_C, FG=pF2_G, yvar=yp )
  yp <- "KHEXP(3)"  ; plotimpacts_CG.Nfert( FC=pF2_C, FG=pF2_G, yvar=yp )
  yp <- "KAC(3)"    ; plotimpacts_CG.Nfert( FC=pF2_C, FG=pF2_G, yvar=yp )
  yp <- "KACEXP(3)" ; plotimpacts_CG.Nfert( FC=pF2_C, FG=pF2_G, yvar=yp )
```
...
```{r XX13, fig.height=4, fig.cap="\\label{fig:XX13}Simulated yields in 2018-19 and 2019-20 vs. long-term average yields in the same simulations."}
  ...
  plot  ( oF2_C[,"harvDM_f_hay"], oF2_C[,"harvDM_f_ha1819"],
          main="Costa Rica (sim.)",
          xlab="mean yield", ylab="kg DM ha-1 y-1")
  points( oF2_C[,"harvDM_f_hay"], oF2_C[,"harvDM_f_ha1920"], col="red" )
  abline( 1, 1, lty=2 )
  legend( "bottomright",legend=c("y=x","18/19","19/20"),
          lty=c(2,NA,NA), pch=c(NA,1,1), col=c("black","black","red"),
          bg="transparent" )
  
  plot  ( oF2_G[,"harvDM_f_hay"], oF2_G[,"harvDM_f_ha1819"],
          main="Guatemala (sim.)",
          xlab="mean yield", ylab="yield 2018/19")
  points( oF2_G[,"harvDM_f_hay"], oF2_G[,"harvDM_f_ha1920"], col="red" )
  abline( 1, 1, lty=2 )
  legend( "bottomright",legend=c("y=x","18/19","19/20"),
          lty=c(2,NA,NA), pch=c(NA,1,1), col=c("black","black","red"),
          bg="transparent" )
...
```{r}
  ...
  y1920.err_C <- oF2_C[,"harvDM_f_ha1819"] - df_C.f$yield1920
  y1819.err_G <- oF2_G[,"harvDM_f_ha1819"] - df_G.f$yield1819
  y1920.err_G <- oF2_G[,"harvDM_f_ha1819"] - df_G.f$yield1920
```
