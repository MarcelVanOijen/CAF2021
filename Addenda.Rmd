...
```{r}
  ...
  file.create( file.parBC_C )
  
  if_C1.E <- which(Tnames_C[,1]=="E") ; if_C1.I <- which(Tnames_C[,1]=="I")
  if_C2.B <- which(Tnames_C[,2]=="B") ; if_C2.A <- which(Tnames_C[,2]=="A")
  if_C3.G <- which(Tnames_C[,3]=="G") ; if_C3.C <- which(Tnames_C[,3]=="C")

  file.init_C.E <- paste0(dir.init,"inititialise_CAF2021_C_f", if_C.1E[1], ".R")
  file.init_C.I <- paste0(dir.init,"inititialise_CAF2021_C_f", if_C.1I[1], ".R")
  file.init_C.B <- paste0(dir.init,"inititialise_CAF2021_C_f", if_C.2B[1], ".R")
  file.init_C.A <- paste0(dir.init,"inititialise_CAF2021_C_f", if_C.2A[1], ".R")
  file.init_C.G <- paste0(dir.init,"inititialise_CAF2021_C_f", if_C.3G[1], ".R")
  file.init_C.C <- paste0(dir.init,"inititialise_CAF2021_C_f", if_C.3C[1], ".R")

  source( file.init_C )
  pAll.def       <- round( params[ipAll], 3 )
  pAll.min       <- round( pAll.def * 0.5, 3 )
  ipfr           <- 2:3 # Fractions in [0,1]
  pAll.max       <- pAll.def * 2
  pAll.max[ipfr] <- round( pAll.def[ipfr] + 0.5*(1-pAll.def[ipfr]), 3 )
...
```{r}
  ...
  file.create( file.parBC_G )
  
  if_G1.I <- which(Tnames_G[,1]=="I")
  if_G2.B <- which(Tnames_G[,2]=="B") ; if_G2.A <- which(Tnames_G[,2]=="A")
  if_G3.G <- which(Tnames_G[,3]=="G") ; if_G3.C <- which(Tnames_G[,3]=="C")

  file.init_G   <- paste0(dir.init,"inititialise_CAF2021_G_f", 1        , ".R")
  file.init_G.I <- paste0(dir.init,"inititialise_CAF2021_G_f", if_G1.I[1], ".R")
  file.init_G.B <- paste0(dir.init,"inititialise_CAF2021_G_f", if_G2.B[1], ".R")
  file.init_G.A <- paste0(dir.init,"inititialise_CAF2021_G_f", if_G2.A[1], ".R")
  file.init_G.G <- paste0(dir.init,"inititialise_CAF2021_G_f", if_G3.G[1], ".R")
  file.init_G.C <- paste0(dir.init,"inititialise_CAF2021_G_f", if_G3.C[1], ".R")
...
```{r, eval=F}
  ...
  for(fi in 1:n_C.f) {
    ...
    if((!is.na(y1819))) {
      cat( paste0("harvDM_f_hay\t2019\t181\t",y1819,"\t",0.25,"\n"),
           file=file.f ) }
    if((!is.na(y1920))) {
      cat( paste0("harvDM_f_hay\t2020\t181\t",y1920,"\t",0.25,"\n"),
           file=file.f, append=TRUE ) }
    if((!is.na(shade))) {
      cat( paste0("Shade_f\t2019\t300\t"     ,shade,"\t",0.50,"\n"),
           file=file.f, append=TRUE ) }
  }
```
...
```{r}
  # source('BC_CAF2021_C.R')
  # matMAP_C <- matMAP
  matMAP_C <- readRDS( "CAF2021_parMAP_00_37.rds" )

  # source('BC_CAF2021_G.R')
  # matMAP_G <- matMAP
  matMAP_G <- readRDS( "CAF2021_parMAP_05_31.rds" )
...
```{r}
  # KAC
  range( matMAP_C["KAC(1)",if_C1.E] )    ; range( matMAP_C["KAC(1)",if_C1.I] )
  range( matMAP_C["KAC(2)",if_C2.B] )    ; range( matMAP_C["KAC(2)",if_C2.A] )
  range( matMAP_C["KAC(3)",if_C3.G] )    ; range( matMAP_C["KAC(3)",if_C3.C] )
  
  # KACEXP
  range( matMAP_C["KACEXP(1)",if_C1.E] ) ; range( matMAP_C["KACEXP(1)",if_C1.I] )
  range( matMAP_C["KACEXP(2)",if_C2.B] ) ; range( matMAP_C["KACEXP(2)",if_C2.A] )
  range( matMAP_C["KACEXP(3)",if_C3.G] ) ; range( matMAP_C["KACEXP(3)",if_C3.C] )

  # KH
  range( matMAP_C["KH(1)",if_C1.E] )     ; range( matMAP_C["KH(1)",if_C1.I] )
  range( matMAP_C["KH(2)",if_C2.B] )     ; range( matMAP_C["KH(2)",if_C2.A] )
  range( matMAP_C["KH(3)",if_C3.G] )     ; range( matMAP_C["KH(3)",if_C3.C] )

  # KHEXP
  range( matMAP_C["KHEXP(1)",if_C1.E] )  ; range( matMAP_C["KHEXP(1)",if_C1.I] )
  range( matMAP_C["KHEXP(2)",if_C2.B] )  ; range( matMAP_C["KHEXP(2)",if_C2.A] )
  range( matMAP_C["KHEXP(3)",if_C3.G] )  ; range( matMAP_C["KHEXP(3)",if_C3.C] )
```

```{r}
  # KAC
                                           range( matMAP_G["KAC(1)",if_G1.I] )
  range( matMAP_G["KAC(2)",if_G2.B] )    ; range( matMAP_G["KAC(2)",if_G2.A] )
  range( matMAP_G["KAC(3)",if_G3.G] )    ; range( matMAP_G["KAC(3)",if_G3.C] )
  
  # KACEXP
                                           range( matMAP_G["KACEXP(1)",if_G1.I] )
  range( matMAP_G["KACEXP(2)",if_G2.B] ) ; range( matMAP_G["KACEXP(2)",if_G2.A] )
  range( matMAP_G["KACEXP(3)",if_G3.G] ) ; range( matMAP_G["KACEXP(3)",if_G3.C] )

  # KH
                                           range( matMAP_G["KH(1)",if_G1.I] )
  range( matMAP_G["KH(2)",if_G2.B] )     ; range( matMAP_G["KH(2)",if_G2.A] )
  range( matMAP_G["KH(3)",if_G3.G] )     ; range( matMAP_G["KH(3)",if_G3.C] )

  # KHEXP
                                           range( matMAP_G["KHEXP(1)",if_G1.I] )
  range( matMAP_G["KHEXP(2)",if_G2.B] )  ; range( matMAP_G["KHEXP(2)",if_G2.A] )
  range( matMAP_G["KHEXP(3)",if_G3.G] )  ; range( matMAP_G["KHEXP(3)",if_G3.C] )
```
...
```{r list_outF_C}
  ...
  for(fi in 1:n_C.f) {
    ...
    for(iF in 1:nF_C) {
      ...
      if( iF==3 ) {
        params <- matMAP_C[,fi]
        CSOM0  <- params[ which(names_params=="CSOM0") ]
        params <- set_par( "CSOM0", CSOM0*2  , params )
        # params <- set_par( "FCSOMF0"  , 0.75, params )
        # params <- set_par( "NFERTMULT", 2   , params )
        # params <- set_par( "RAINMULT" , 2   , params )
      }
      outF <- run_model( params, matrix_weather, calendar_fert,
        calendar_prunC, calendar_prunT, calendar_thinT, NDAYS )
      mat_parF[iF,1+(1:length(names_params))] <- params
...
```{r, echo=T}
  summary( oF2_C[if_C1.E,"CAtree_t_1b"], digits=3 ) # Erythrina final two years
  summary( oF2_C[if_C1.I,"CAtree_t_1b"], digits=3 ) # Inga final two years
  summary( oF2_C[if_C2.B,"CAtree_t_2b"], digits=3 ) # Banana final two years
  summary( oF2_C[if_C2.A,"CAtree_t_2b"], digits=3 ) # Avocado final two years
  summary( oF2_C[if_C3.G,"CAtree_t_3b"], digits=3 ) # Grevillea final two years
  summary( oF2_C[if_C3.C,"CAtree_t_3b"], digits=3 ) # Cordia final two years

  summary( oF2_G[if_G1.I,"CAtree_t_1b"], digits=3 ) # Inga final two years
  summary( oF2_G[if_G2.B,"CAtree_t_2b"], digits=3 ) # Banana final two years
  summary( oF2_G[if_G2.A,"CAtree_t_2b"], digits=3 ) # Avocado final two years
  summary( oF2_G[if_G3.G,"CAtree_t_3b"], digits=3 ) # Grevillea final two years
  summary( oF2_G[if_G3.C,"CAtree_t_3b"], digits=3 ) # Cordia final two years
```

```{r, echo=T}
# Costa Rica, MAP
  summary( oF2_C[if_C1.E,"H_t_1b"], digits=3 ) # Erythrina final two years
  summary( oF2_C[if_C1.I,"H_t_1b"], digits=3 ) # Inga final two years
  summary( oF2_C[if_C2.B,"H_t_2b"], digits=3 ) # Banana final two years
  summary( oF2_C[if_C2.A,"H_t_2b"], digits=3 ) # Avocado final two years
  summary( oF2_C[if_C3.G,"H_t_3b"], digits=3 ) # Grevillea final two years
  summary( oF2_C[if_C3.C,"H_t_3b"], digits=3 ) # Cordia final two years

# Guatemala, MAP
  summary( oF2_G[if_G1.I,"H_t_1b"], digits=3 ) # Inga final two years
  summary( oF2_G[if_G2.B,"H_t_2b"], digits=3 ) # Banana final two years
  summary( oF2_G[if_G2.A,"H_t_2b"], digits=3 ) # Avocado final two years
  summary( oF2_G[if_G3.G,"H_t_3b"], digits=3 ) # Grevillea final two years
  summary( oF2_G[if_G3.C,"H_t_3b"], digits=3 ) # Cordia final two years
```
...
```{r list_outF_G}
  ...
  # source('initialisation/set_CAF2021_CG.R')
  for(fi in 1:n_G.f) {
    # matrix_weather <- read_listweather_CAF( list_w=df_G.f$weather.WC,
    #   f=fi, y1=year_start, d1=doy_start, nd=NDAYS )
    # params         <- set_par_CG  ( df_G.f, fi, params, 'G' )
    # calendar_fert  <- set_fert_CG ( df_G.f, fi )
    # calendar_prunT <- set_prunT_CG( df_G.f, fi )
    # calendar_thinT <- set_thinT_CG( df_G.f, fi )
