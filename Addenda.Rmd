...
```{r}
  ...
  pAll     <- c("CNSOM0MULT","CSOM0MULT","FWCWPMULT","WCSTMULT",
                "SHADETARGETMULT","NFERTMULT")
...
```{r}
  ...
  file.init_C.E <- paste0(dir.init,"inititialise_CAF2021_C_f", if_C1.E[1], ".R")
  file.init_C.I <- paste0(dir.init,"inititialise_CAF2021_C_f", if_C1.I[1], ".R")
  file.init_C.B <- paste0(dir.init,"inititialise_CAF2021_C_f", if_C2.B[1], ".R")
  file.init_C.A <- paste0(dir.init,"inititialise_CAF2021_C_f", if_C2.A[1], ".R")
  file.init_C.G <- paste0(dir.init,"inititialise_CAF2021_C_f", if_C3.G[1], ".R")
  file.init_C.C <- paste0(dir.init,"inititialise_CAF2021_C_f", if_C3.C[1], ".R")

  source( file.init_C )
  pAll.def         <- round( params[ipAll] , 3 )
  pAll.min         <- round( pAll.def * 0.5, 3 )
  pAll.max         <- round( pAll.def * 1.5, 3 )
  pAll.max[c(3,5)] <- 1.1
...
```{r}
  ...
  matMAP_C <- matMAP
  # matMAP_C.old <- readRDS( "CAF2021_parMAP_00_37.rds" )
  # matMAP_C     <- update_matMAP( matMAP_C.old,
  #                                'initialisation/set_CAF2021_CG.R' )

  source('BC_CAF2021_G.R')
  matMAP_G <- matMAP
  # matMAP_G.old <- readRDS( "CAF2021_parMAP_05_31.rds" )
...
```{r list_outF_C}
  ...
  for(fi in 1:n_C.f) {
    ...
    for(iF in 1:nF_C) {
      ...
      if( iF==3 ) {
        params <- matMAP_C[,fi]
        params <- set_par( "CNSOM0MULT"     , 0.5, params )
        # params <- set_par( "CSOM0MULT"      , 0.5, params )
        # params <- set_par( "FWCWPMULT"      , 0.5, params )
        # params <- set_par( "WCSTMULT"       , 1.5, params )
        # params <- set_par( "SHADETARGETMULT", 1.5, params )
        # params <- set_par( "NFERTMULT"      , 0.5, params )
        # params <- set_par( "RAINMULT"       , 0.5, params )
      }
