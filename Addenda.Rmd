...
```{r}
  ...
  df_soil2.f <- df_soil2.f0 %>%
    ...
    select( code, Cperc.soil.00_26, Nperc.soil.00_26, sand, silt, clay )
```
...
```{r}
  df_C.f <- df_C.f %>%
    select ( ., -c(DM.soil,CN.soil,C.soil,N.soil) ) %>%
...
```{r}
  vars_Summary <- c(
    ...
    "NfixT_fb"       , "Nleaching_fb"   , "Crunoff_fb"   , "Csoil_fb"     ,
...
```{r}
  ...
  ip_FCSOMF0     <- which( names_params=="FCSOMF0"     )
  ...
  ip_NFERTMULT   <- which( names_params== "NFERTMULT" )
  ip_RAINMULT    <- which( names_params== "RAINMULT" )
```
...
```{r list_outF_C}
  ...
  for(fi in 1:n_C.f) {
    ...
    for(iF in 1:nF_C) {
      params_fi <- parMAP[,31]
      params_fi <- set_parT( "C", densE_C, densI_C, densB_C, densA_C,
                                  densG_C, densC_C, p.old=params_fi )
      ...
      if( iF==2 ) {
        params_fi[ ip_CSOM0 ] <- C_C * 0.5
        # params_fi[ ip_FCSOMF0 ] <- 0.25
        # params_fi[ ip_NFERTMULT ] <- 0.5
        # params_fi[ ip_RAINMULT ] <- 0.5
      }
      if( iF==3 ) {
        params_fi[ ip_CSOM0 ] <- C_C * 2
        # params_fi[ ip_FCSOMF0 ] <- 0.75
        # params_fi[ ip_NFERTMULT ] <- 2
        # params_fi[ ip_RAINMULT ] <- 2
      }
...
```{r}
  ...
  F3_C <- dfoutFSummary( df_C.f_sim, Fi=3 )
```

```{r}
  par( mfrow=c(1,3), mar=c(4,4,1,1) )

  plot( F1_C[,"Csoil_fb"], df_C.f$C.soil ) ; abline(a=0,b=1,lty=2)
  plot( F2_C[,"Csoil_fb"], df_C.f$C.soil ) ; abline(a=0,b=1,lty=2)
  plot( F3_C[,"Csoil_fb"], df_C.f$C.soil ) ; abline(a=0,b=1,lty=2)
```

```{r}
  par( mfrow=c(3,3), mar=c(4,4,1,1) )

  plot( F1_C[,"Shade_fa"], df_C.f$shade  ) ; abline(a=0,b=1,lty=2)
  plot( F2_C[,"Shade_fa"], df_C.f$shade  ) ; abline(a=0,b=1,lty=2)
  plot( F3_C[,"Shade_fa"], df_C.f$shade  ) ; abline(a=0,b=1,lty=2)

  plot( F1_C[,"Shade_f"], df_C.f$shade  ) ; abline(a=0,b=1,lty=2)
  plot( F2_C[,"Shade_f"], df_C.f$shade  ) ; abline(a=0,b=1,lty=2)
  plot( F3_C[,"Shade_f"], df_C.f$shade  ) ; abline(a=0,b=1,lty=2)

  plot( F1_C[,"Shade_fb"], df_C.f$shade  ) ; abline(a=0,b=1,lty=2)
  plot( F2_C[,"Shade_fb"], df_C.f$shade  ) ; abline(a=0,b=1,lty=2)
  plot( F3_C[,"Shade_fb"], df_C.f$shade  ) ; abline(a=0,b=1,lty=2)
```

```{r XX14C, fig.height=4, fig.cap="\\label{fig:XX14C}Observed vs. simulated yields in 2018-19 and 2019-20 at the farms in Costa Rica."}
  par( mfrow=c(2,3), mar=c(4,3,2,2), mgp=c(2,1,0) )

  x=F1_C[,"harvDM_f_ha1819"] ; y=df_C.f$yield1819
  ...
  x=F2_C[,"harvDM_f_ha1819"] ; y=df_C.f$yield1819
    plotxylm("Yields Costa Rica 18/19", orig=F) ; abline(a=0,b=1,lty=2)
    abline(h=mean(y,na.rm=T),lty=2,col="red") ; abline(v=mean(x),lty=2,col="red") 
  x=F3_C[,"harvDM_f_ha1819"] ; y=df_C.f$yield1819
    plotxylm("Yields Costa Rica 18/19", orig=F) ; abline(a=0,b=1,lty=2)
    abline(h=mean(y,na.rm=T),lty=2,col="red") ; abline(v=mean(x),lty=2,col="red") 
    
  x=F1_C[,"harvDM_f_ha1920"] ; y=df_C.f$yield1920
    plotxylm("Yields Costa Rica 19/20", orig=F) ; abline(a=0,b=1,lty=2)
    abline(h=mean(y,na.rm=T),lty=2,col="red") ; abline(v=mean(x),lty=2,col="red") 
  x=F2_C[,"harvDM_f_ha1920"] ; y=df_C.f$yield1920
    plotxylm("Yields Costa Rica 19/20", orig=F) ; abline(a=0,b=1,lty=2)
    abline(h=mean(y,na.rm=T),lty=2,col="red") ; abline(v=mean(x),lty=2,col="red") 
  x=F3_C[,"harvDM_f_ha1920"] ; y=df_C.f$yield1920
    plotxylm("Yields Costa Rica 19/20", orig=F) ; abline(a=0,b=1,lty=2)
    abline(h=mean(y,na.rm=T),lty=2,col="red") ; abline(v=mean(x),lty=2,col="red") 
```
...
```{r}
  ...
  iptest   <- which( names_params %in% c(
    "SHADETARGET",
    "FWT(1)"      ,"FWT(2)"      ,"FWT(3)"      ,
    "FST(1)"      ,"FST(2)"      ,"FST(3)"      ,
    "FPT(1)"      ,"FPT(2)"      ,"FPT(3)"      ,
    "KH(1)"       ,"KH(2)"       ,"KH(3)"       ,
    "KAC(1)"      ,"KAC(2)"      ,"KAC(3)"      ,
    "TCBT(1)"     ,"TCBT(2)"     ,"TCBT(3)"     ,
    "TCRT(1)"     ,"TCRT(2)"     ,"TCRT(3)"     ,
    "TCST(1)"     ,"TCST(2)"     ,"TCST(3)"     ,
    "TOPTT(1)"    ,"TOPTT(2)"    ,"TOPTT(3)"    ,
    "TTOLT(1)"    ,"TTOLT(2)"    ,"TTOLT(3)"    ,
    "TREEDENS0(1)","TREEDENS0(2)","TREEDENS0(3)",
    "HMAX(1)"     ,"HMAX(2)"     ,"HMAX(3)"     ) )
```

```{r, echo=T}
  ptest <- params_fi[iptest] ; names(ptest) <- names_params[iptest]
...
```{r}
  # ierr <- which( df_C.f$shade > 0.2 & F1_C[,"Shade_f"] < 0.1 )
  # 
  # ierr
...
```{r list_outF_G}
  ...
  # for(fi in 1:65) {
    ...
    for(iF in 1:nF_G) {
      params_fi <- parMAP[,31]
      params_fi <- set_parT( "G", densE_G, densI_G, densB_G, densA_G,
                                  densG_G, densC_G, p.old=params_fi )
      ...
      if( iF==2 ) {
        params_fi[ ip_CSOM0 ] <- C_G * 0.5
        # params_fi[ ip_FCSOMF0 ] <- 0.25
        # params_fi[ ip_NFERTMULT ] <- 0.5
        # params_fi[ ip_RAINMULT ] <- 0.5
      }
      if( iF==3 ) {
        params_fi[ ip_CSOM0 ] <- C_G * 2
        # params_fi[ ip_FCSOMF0 ] <- 0.75
        # params_fi[ ip_NFERTMULT ] <- 2
        # params_fi[ ip_RAINMULT ] <- 2
      }
...
```{r}
  ...
  F3_G <- dfoutFSummary( df_G.f_sim, Fi=3 )
```

```{r}
  par( mfrow=c(1,3), mar=c(4,4,1,1) )

  plot( F1_G[,"Csoil_fb"], df_G.f$C.soil ) ; abline(a=0,b=1,lty=2)
  plot( F2_G[,"Csoil_fb"], df_G.f$C.soil ) ; abline(a=0,b=1,lty=2)
  plot( F3_G[,"Csoil_fb"], df_G.f$C.soil ) ; abline(a=0,b=1,lty=2)
```

```{r}
  par( mfrow=c(3,3), mar=c(4,4,1,1) )

  plot( F1_G[,"Shade_fa"], df_G.f$shade  ) ; abline(a=0,b=1,lty=2)
  plot( F2_G[,"Shade_fa"], df_G.f$shade  ) ; abline(a=0,b=1,lty=2)
  plot( F3_G[,"Shade_fa"], df_G.f$shade  ) ; abline(a=0,b=1,lty=2)

  plot( F1_G[,"Shade_f" ], df_G.f$shade  ) ; abline(a=0,b=1,lty=2)
  plot( F2_G[,"Shade_f" ], df_G.f$shade  ) ; abline(a=0,b=1,lty=2)
  plot( F3_G[,"Shade_f" ], df_G.f$shade  ) ; abline(a=0,b=1,lty=2)

  plot( F1_G[,"Shade_fb"], df_G.f$shade  ) ; abline(a=0,b=1,lty=2)
  plot( F2_G[,"Shade_fb"], df_G.f$shade  ) ; abline(a=0,b=1,lty=2)
  plot( F3_G[,"Shade_fb"], df_G.f$shade  ) ; abline(a=0,b=1,lty=2)
```

```{r XX14G, fig.height=4, fig.cap="\\label{fig:XX14G}Observed vs. simulated yields in 2018-19 and 2019-20 at the farms in Guatemala."}
  par( mfrow=c(2,3), mar=c(4,2,2,2), mgp=c(2,1,0) )

  x=F1_G[,"harvDM_f_ha1819"] ; y=df_G.f$yield1819
    plotxylm("Yields Guatemala 18/19" , orig=F) ; abline(a=0,b=1,lty=2)
    abline(h=mean(y),lty=2,col="red") ; abline(v=mean(x),lty=2,col="red") 
  x=F2_G[,"harvDM_f_ha1819"] ; y=df_G.f$yield1819
    plotxylm("Yields Guatemala 18/19" , orig=F) ; abline(a=0,b=1,lty=2)
    abline(h=mean(y),lty=2,col="red") ; abline(v=mean(x),lty=2,col="red") 
  x=F3_G[,"harvDM_f_ha1819"] ; y=df_G.f$yield1819
    plotxylm("Yields Guatemala 18/19" , orig=F) ; abline(a=0,b=1,lty=2)
    abline(h=mean(y),lty=2,col="red") ; abline(v=mean(x),lty=2,col="red") 
    
  x=F1_G[,"harvDM_f_ha1920"] ; y=df_G.f$yield1920
    plotxylm("Yields Guatemala 19/20" , orig=F) ; abline(a=0,b=1,lty=2)
    abline(h=mean(y,na.rm=T),lty=2,col="red") ; abline(v=mean(x),lty=2,col="red") 
  x=F2_G[,"harvDM_f_ha1920"] ; y=df_G.f$yield1920
    plotxylm("Yields Guatemala 19/20" , orig=F) ; abline(a=0,b=1,lty=2)
    abline(h=mean(y,na.rm=T),lty=2,col="red") ; abline(v=mean(x),lty=2,col="red") 
  x=F3_G[,"harvDM_f_ha1920"] ; y=df_G.f$yield1920
    plotxylm("Yields Guatemala 19/20" , orig=F) ; abline(a=0,b=1,lty=2)
    abline(h=mean(y,na.rm=T),lty=2,col="red") ; abline(v=mean(x),lty=2,col="red") 
```

```{r}
  plotimpacts_CG.Nfert <- function( FC=F1_C, FG=F1_G, yvar="y.sim" ){
    xC=df_C.f$Nfert ; xG=df_G.f$Nfert ; yC=FC[,yvar] ; yG=FG[,yvar]
    plot  ( xC, yC,
      xlab="Nfert", ylab="",
...
```{r XX21, fig.height=7.5, fig.cap="\\label{fig:XX21}XX21."}
  par( mfrow=c(4,3), mar=c(4,2,2,2), mgp=c(2,1,0) )

  yv <- "fTranT_t_1a"   ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )
  yv <- "fTranT_t_1b"   ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )

  yv <- "fNgrowth_t_1a" ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )
  
  yv <- "fNgrowth_t_1b" ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )
```
...
```{r XX22, fig.height=7.5, fig.cap="\\label{fig:XX22}XX22."}
  ...
  yv <- "fTranT_t_2a"   ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )
  yv <- "fTranT_t_2b"   ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )

  yv <- "fNgrowth_t_2a" ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )
  
  yv <- "fNgrowth_t_2b" ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )
```

See Fig. \ref{fig:XX23}.

```{r XX23, fig.height=7.5, fig.cap="\\label{fig:XX23}XX23."}
  par( mfrow=c(4,3), mar=c(4,2,2,2), mgp=c(2,1,0) )

  yv <- "fTranT_t_3a"   ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )
  yv <- "fTranT_t_3b"   ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )

  yv <- "fNgrowth_t_3a" ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )
  
  yv <- "fNgrowth_t_3b" ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                          plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )
```
...
```{r XX24, fig.height=7.5, fig.cap="\\label{fig:XX24}XX24."}
  ...
  yv <- "H_t_1a"      ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )
  yv <- "H_t_1b"      ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )

  yv <- "CAtree_t_1a" ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )
  
  yv <- "CAtree_t_1b" ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )
```
...
```{r XX25, fig.height=7.5, fig.cap="\\label{fig:XX25}XX25."}
  ...
  yv <- "H_t_2a"      ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )
  yv <- "H_t_2b"      ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )

  yv <- "CAtree_t_2a" ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )
  
  yv <- "CAtree_t_2b" ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )
```

See Fig. \ref{fig:XX26}.

```{r XX26, fig.height=7.5, fig.cap="\\label{fig:XX26}XX26."}
  par( mfrow=c(4,3), mar=c(4,2,2,2), mgp=c(2,1,0) )

  yv <- "H_t_3a"      ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )
  yv <- "H_t_3b"      ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )

  yv <- "CAtree_t_3a" ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )
  
  yv <- "CAtree_t_3b" ; plotimpacts_CG.Nfert( FC=F1_C, FG=F1_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F2_C, FG=F2_G, yvar=yv )
                        plotimpacts_CG.Nfert( FC=F3_C, FG=F3_G, yvar=yv )
```

```{r, echo=T}
  print(mean(df_C.f$yield1819))
  cat( mean(F1_C[,"harvDM_f_ha1819"]), mean(F2_C[,"harvDM_f_ha1819"]),
       mean(F3_C[,"harvDM_f_ha1819"]) )
       
  mean(df_C.f$yield1920,na.rm=T)
  cat( mean(F1_C[,"harvDM_f_ha1920"]), mean(F2_C[,"harvDM_f_ha1920"]),
       mean(F3_C[,"harvDM_f_ha1920"]) )
                                   
  print(mean(df_G.f$yield1819))
  cat( mean(F1_G[,"harvDM_f_ha1819"]), mean(F2_G[,"harvDM_f_ha1819"]),
       mean(F3_G[,"harvDM_f_ha1819"]) )

  print( mean(df_G.f$yield1920,na.rm=T))
  cat( mean(F1_G[,"harvDM_f_ha1920"]), mean(F2_G[,"harvDM_f_ha1920"]),
       mean(F3_G[,"harvDM_f_ha1920"]) )
```
...
```{r}
  # ierr <- which( df_G.f$shade > 0.8 & F1_G[,"Shade_f"] < 0.05 )
  # 
  # params_fi[ip_WCST] * params_fi[ c(ip_FWCWP,ip_FWCFC) ] # Farm 79: 0.207,0.364
  # 
  # ierr
...
```{r XX13, fig.height=4, fig.cap="\\label{fig:XX13}Simulated yields in 2018-19 and 2019-20 vs. long-term average yields in the same simulations."}
  ...
  plot  ( F1_C[,"harvDM_f_hay"], F1_C[,"harvDM_f_ha1819"],
          main="Costa Rica (sim.)",
          xlab="mean yield", ylab="kg DM ha-1 y-1")
  points( F1_C[,"harvDM_f_hay"], F1_C[,"harvDM_f_ha1920"], col="red" )
  abline( 1, 1, lty=2 )
  legend( "bottomright",legend=c("y=x","18/19","19/20"),
          lty=c(2,NA,NA), pch=c(NA,1,1), col=c("black","black","red"),
          bg="transparent" )
  
  plot  ( F1_G[,"harvDM_f_hay"], F1_G[,"harvDM_f_ha1819"],
          main="Guatemala (sim.)",
          xlab="mean yield", ylab="yield 2018/19")
  points( F1_G[,"harvDM_f_hay"], F1_G[,"harvDM_f_ha1920"], col="red" )
  abline( 1, 1, lty=2 )
  legend( "bottomright",legend=c("y=x","18/19","19/20"),
          lty=c(2,NA,NA), pch=c(NA,1,1), col=c("black","black","red"),
          bg="transparent" )
...
```{r}
  listmat_nt_C[[1]] <- t( sapply( 1:nnt1_C, function(i){ cbind(
    ...
    df_C.f_sim[int1_C,]$outF     [[i]][,"Nleaching_fb"   ][1],
    df_C.f_sim[int1_C,]$outF     [[i]][,"Crunoff_fb"     ][1],
    df_C.f_sim[int1_C,]$outF     [[i]][,"Csoil_fb"       ][1]
  ) } ) )
  ...
  listmat_nt_C[[2]] <- t( sapply( 1:nnt2_C, function(i){ cbind(
    ...
    df_C.f_sim[int2_C,]$outF     [[i]][,"Nleaching_fb"   ][1],
    df_C.f_sim[int2_C,]$outF     [[i]][,"Crunoff_fb"     ][1],
    df_C.f_sim[int2_C,]$outF     [[i]][,"Csoil_fb"       ][1]
  ) } ) )
  ...
  listmat_nt_C[[3]] <- t( sapply( 1:nnt3_C, function(i){ cbind(
    ...
    df_C.f_sim[int3_C,]$outF     [[i]][,"Nleaching_fb"   ][1],
    df_C.f_sim[int3_C,]$outF     [[i]][,"Crunoff_fb"     ][1],
    df_C.f_sim[int3_C,]$outF     [[i]][,"Csoil_fb"       ][1]
  ) } ) )
...
```{r}
  listmat_nt_G[[1]] <- t( sapply( 1:nnt1_G, function(i){ cbind(
    ...
    df_G.f_sim[int1_G,]$outF     [[i]][,"Nleaching_fb"   ][1],
    df_G.f_sim[int1_G,]$outF     [[i]][,"Crunoff_fb"     ][1],
    df_G.f_sim[int1_G,]$outF     [[i]][,"Csoil_fb"       ][1]
  ) } ) )
  ...
  listmat_nt_G[[2]] <- t( sapply( 1:nnt2_G, function(i){ cbind(
    ...
    df_G.f_sim[int2_G,]$outF     [[i]][,"Nleaching_fb"   ][1],
    df_G.f_sim[int2_G,]$outF     [[i]][,"Crunoff_fb"     ][1],
    df_G.f_sim[int2_G,]$outF     [[i]][,"Csoil_fb"       ][1]
  ) } ) )
  ...
  listmat_nt_G[[3]] <- t( sapply( 1:nnt3_G, function(i){ cbind(
    ...
    df_G.f_sim[int3_G,]$outF     [[i]][,"Nleaching_fb"   ][1],
    df_G.f_sim[int3_G,]$outF     [[i]][,"Crunoff_fb"     ][1],
    df_G.f_sim[int3_G,]$outF     [[i]][,"Csoil_fb"       ][1]
  ) } ) )
