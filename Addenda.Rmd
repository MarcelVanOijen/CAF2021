...
```{r}
  ...
  if_C.E <- which(Tnames_C[,1]=="E") ; if_C.I <- which(Tnames_C[,1]=="I")
  if_C.B <- which(Tnames_C[,2]=="B") ; if_C.A <- which(Tnames_C[,2]=="A")
  if_C.G <- which(Tnames_C[,3]=="G") ; if_C.C <- which(Tnames_C[,3]=="C")

  file.init_C.E <- paste0(dir.init,"inititialise_CAF2021_C_f", if_C.E[1], ".R")
  file.init_C.I <- paste0(dir.init,"inititialise_CAF2021_C_f", if_C.I[1], ".R")
  file.init_C.B <- paste0(dir.init,"inititialise_CAF2021_C_f", if_C.B[1], ".R")
  file.init_C.A <- paste0(dir.init,"inititialise_CAF2021_C_f", if_C.A[1], ".R")
  file.init_C.G <- paste0(dir.init,"inititialise_CAF2021_C_f", if_C.G[1], ".R")
  file.init_C.C <- paste0(dir.init,"inititialise_CAF2021_C_f", if_C.C[1], ".R")
...
```{r}
  ...
  if_G.I <- which(Tnames_G[,1]=="I")
  if_G.B <- which(Tnames_G[,2]=="B") ; if_G.A <- which(Tnames_G[,2]=="A")
  if_G.G <- which(Tnames_G[,3]=="G") ; if_G.C <- which(Tnames_G[,3]=="C")

  file.init_G   <- paste0(dir.init,"inititialise_CAF2021_G_f", 1        , ".R")
  file.init_G.I <- paste0(dir.init,"inititialise_CAF2021_G_f", if_G.I[1], ".R")
  file.init_G.B <- paste0(dir.init,"inititialise_CAF2021_G_f", if_G.B[1], ".R")
  file.init_G.A <- paste0(dir.init,"inititialise_CAF2021_G_f", if_G.A[1], ".R")
  file.init_G.G <- paste0(dir.init,"inititialise_CAF2021_G_f", if_G.G[1], ".R")
  file.init_G.C <- paste0(dir.init,"inititialise_CAF2021_G_f", if_G.C[1], ".R")
...
## After BC: MAP parameter vector

```{r}
  # matMAP_C <- readRDS( "CAF2021_parMAP_23_22.rds" )
  # matMAP_G <- readRDS( "CAF2021_parMAP_06_00.rds" )
```
...
```{r list_outF_C}
  ...
  for(fi in 1:n_C.f) {
    ...
    for(iF in 1:nF_C) {
      if( iF==2 ) {
        params <- params1
        params <- set_par( "CSOM0", CSOM0*0.5, params )
        # params <- set_par( "FCSOMF0"  , 0.25, params )
