...
```{r}
  ...
  matMAP_C <- readRDS( "CAF2021_parMAP_10_33.rds" )
  matMAP_G <- readRDS( "CAF2021_parMAP_09_54.rds" )
```
  ...
  for(fi in 1:n_C.f) {
    ...
    for(iF in 1:nF_C) {
      ...
      if( iF==3 ) {
        ...
        # calendar_fert[,3] <- 70 + calendar_fert[,3]
        # calendar_fert[,3] <- pmax(70,calendar_fert[,3])
...
```{r, eval=F}
  unique( matMAP_G["KAC(2)"   ,if_G.2B] )
  unique( matMAP_G["KACEXP(2)",if_G.2B] )
  unique( matMAP_G["KH(2)"    ,if_G.2B] )
  unique( matMAP_G["KHEXP(2)" ,if_G.2B] )
```
