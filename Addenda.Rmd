...
```{r, eval=F}
  sapply( paste0(dir.init,"inititialise_CAF2021_G_f",1:n_G.f,".R"), file.create)
...
```{r, eval=F}
  ...
  for(fi in 1:n_C.f) {
    ...
    y1819 <- round(df_C.f$yield1819[fi]) ; y1920 <- round(df_C.f$yield1920[fi])
    if((!is.na(y1819))) {
...
```{r, eval=F}
  ...
  for(fi in 1:n_G.f) {
    ...
    y1819 <- round(df_G.f$yield1819[fi]) ; y1920 <- round(df_G.f$yield1920[fi])
    if((!is.na(y1819))) {
      cat( paste0("harvDM_f_hay\t2019\t181\t",y1819,"\t",0.25,"\n"),
           file=file.f ) }
    if((!is.na(y1920))) {
...
```{r, echo=T}
  ptest <- params[iptest] ; names(ptest) <- names_params[iptest]
