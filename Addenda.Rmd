...
```{r}
  df <- df_C.f_sim
  F1_C <- dfoutFSummary( df, Fi=1 ) ; yF1_C.f_sim <- F1_C[,"harvDM_f_ha1819"]
  F2_C <- dfoutFSummary( df, Fi=2 ) ; yF2_C.f_sim <- F2_C[,"harvDM_f_ha1819"]

  df <- df_G.f_sim
  F1_G <- dfoutFSummary( df, Fi=1 ) ; yF1_G.f_sim <- F1_G[,"harvDM_f_ha1819"]
  F2_G <- dfoutFSummary( df, Fi=2 ) ; yF2_G.f_sim <- F2_G[,"harvDM_f_ha1819"]
```
...
```{r}
  ...
  varnames.listmat <- c( "code","slope","nt","Nfert","shade","y.obs",
    "y.sim","y.sim1819","y.sim1920",
    "D_Csoil","Shade_f","Nleaching_f","Crunoff_f","Csoil_f" )
```
...
```{r}
  listmat_nt_C[[1]] <- t( sapply( 1:nnt1_C, function(i){ cbind(
    ...
    y.sim1819 = df_C.f_sim[int1_C,]$outF [[i]][,"harvDM_f_ha1819"][2],
    y.sim1920 = df_C.f_sim[int1_C,]$outF [[i]][,"harvDM_f_ha1920"][2],
  ...
  listmat_nt_C[[2]] <- t( sapply( 1:nnt2_C, function(i){ cbind(
    ...
    y.sim1819 = df_C.f_sim[int2_C,]$outF [[i]][,"harvDM_f_ha1819"][2],
    y.sim1920 = df_C.f_sim[int2_C,]$outF [[i]][,"harvDM_f_ha1920"][2],
  ...
  listmat_nt_C[[3]] <- t( sapply( 1:nnt3_C, function(i){ cbind(
    ...
    y.sim1819 = df_C.f_sim[int3_C,]$outF [[i]][,"harvDM_f_ha1819"][2],
    y.sim1920 = df_C.f_sim[int3_C,]$outF [[i]][,"harvDM_f_ha1920"][2],
...
```{r}
  listmat_nt_G[[1]] <- t( sapply( 1:nnt1_G, function(i){ cbind(
    ...
    y.sim1819 = df_G.f_sim[int1_G,]$outF [[i]][,"harvDM_f_ha1819"][2],
    y.sim1819 = df_G.f_sim[int1_G,]$outF [[i]][,"harvDM_f_ha1920"][2],
  ...
  listmat_nt_G[[2]] <- t( sapply( 1:nnt2_G, function(i){ cbind(
    ...
    y.sim1819 = df_G.f_sim[int2_G,]$outF [[i]][,"harvDM_f_ha1819"][2],
    y.sim1920 = df_G.f_sim[int2_G,]$outF [[i]][,"harvDM_f_ha1920"][2],
  ...
  listmat_nt_G[[3]] <- t( sapply( 1:nnt3_G, function(i){ cbind(
    ...
    y.sim1819 = df_G.f_sim[int3_G,]$outF [[i]][,"harvDM_f_ha1819"][2],
    y.sim1920 = df_G.f_sim[int3_G,]$outF [[i]][,"harvDM_f_ha1920"][2],
...
```{r XX15, fig.height=7.5, fig.cap="\\label{fig:XX15}Yield-observations for farms in Costa Rica (y.obs; kg DM ha-1 y-1) plus simulations of five variables: Long-term average coffee yield (y.obs; kg DM ha-1 y-1), Change in soil carbon (D_Csoil; kg ha-1 y-1), Average shade level (-), Final year nitrogen leaching (kg N ha-1 y-1), Final year carbon loss in run-off (kg C ha-1 y-1)."}
  ...
  plotimpacts_nt.Nfert( listmat_nt_C, "y.sim1819" )
...
```{r XX16, fig.height=7.5, fig.cap="\\label{fig:XX16}Yield-observations for farms in Guatemala plus simulations of five variables: see previous figure for explanation of variables."}
  ...
  plotimpacts_nt.Nfert( listmat_nt_G, "y.sim1819" )
...
```{r}
  ...
  # plot(yF2_C.f_sim,yF2_C.f_sim1819)
  # plot(yF2_G.f_sim,yF2_G.f_sim1819)
```

```{r XX17, fig.height=4, fig.cap="\\label{fig:XX17}Observed yields at the farms in Costa Rica (left) and Guatemala (right) plotted against long-term average simulated yields 2005-2020. The simulations used farm-specific information on weather, soil, N-fertilisation and number of shade tree species and site-specific shade management."}
  par( mfrow=c(1,2), mar=c(4,2,2,2), mgp=c(2,1,0) )
...
```{r XX18, fig.height=4, fig.cap="\\label{fig:XX18}Observed yields at the farms in Costa Rica (left) and Guatemala (right) plotted against simulated yields for the 2018-2019 harvest season. The simulations used farm-specific information on weather, soil, N-fertilisation and number of shade tree species and site-specific shade management."}
  par( mfrow=c(1,2), mar=c(4,2,2,2), mgp=c(2,1,0) )

  x=yF2_C.f_sim1819 ; y=df_C.f$yield ; plotxylm("Obs. Costa Rica 18/19")

  x=yF2_G.f_sim1819 ; y=df_G.f$yield ; plotxylm("Obs. Guatemala 18/19")
```

```{r, eval=F}
  colnames( outF ) <- outputNames
  d <- (NDAYS_fi-730) : NDAYS_fi
  plot( outF[d,"Time"], outF[d,"harvDM_f_hay"] )
```
...
```{r}
  lonlat_C12.f <- df_C12.f[ c("lon","lat") ]
  lonlat_G12.f <- df_G12.f[ c("lon","lat") ]
  coordsT      <- c( -83.68,  9.89  ) # Longitude, Latitude
  coordsG170   <- c( -90.44, 14.429 ) # Longitude, Latitude
  dist_C12.T   <- distm( coordsT   , lonlat_C12.f ) / 1000
  dist_G12.170 <- distm( coordsG170, lonlat_G12.f ) / 1000
```
...
```{r}
  ...
  x = dist_C12.T   ; y = df_C12.f$yield2 - df_C12.f$yield
  ynna <- which(!is.na(y))
  x <- x[ynna] ; y <- y[ynna]
  plotxylm( "Yield-change vs. dist. Turrialba", ""  )

  x = dist_G12.170 ; y = df_G12.f$yield2 - df_G12.f$yield
  ynna <- which(!is.na(y))
  x <- x[ynna] ; y <- y[ynna]
