...
```{r, include=FALSE}
  ...
  # 2021-03-22: In progress: integration with Github where 2FA is now set up
```
...
```{r, echo=FALSE}
  ...
  knitr::kable( cbind( c(1:10,"...",NPAR-1,NPAR),
                       rbind( head(cbind(names_params,params),n=10),
                              c("...","..."),
                              tail(cbind(names_params,params),n= 2,addrownums=F) ) ),
...
```{r, echo=FALSE}
  ...
  icols        <- c(1:5,NOUT+(-1:0))
  knitr::kable( rbind( head(output_short[,icols]),
                       rep("...",length(icols)),
                       tail(output_short[,icols],n= 2,addrownums=F) ),
...
With our default parameter settings, after about 10 years, we see increasing domination of Land Cover Class (LCC) 4, which is coffee shaded only by the upper-layer trees. Note that the thinning intensity of the upper-layer trees was set at half that of the two lower-layer tree species.
...
```{r}
  ...
  legend( "topleft", col=c("red","black","blue"), lty=1,
          legend=c("sp. 3 (upper layer)",
                   "sp. 1 (lower layer)", "sp. 2 (lower layer)") )
```
...
```{r}
  ...
  legend( "topleft", col=c("red","blue","black"), lty=1,
          legend=c("sp. 3 (upper layer)",
                   "sp. 2 (lower layer)", "sp. 1 (lower layer)") )
```
