---
title: 'CAF2021: Modelling coffee agroforestry systems with up to three shade tree spp.'
author:
- Marcel van Oijen $^{1,*}$
- $^*$ mvano@ceh.ac.uk ; vanoijenmarcel@gmail.com
date: '`r Sys.Date()`'
output:
  pdf_document:
    fig_crop: false
...
```{r, include=FALSE}
# When working at an UKCEH-machine, run one of the following statements
# to knit this document. Elsewhere, you can use RStudio's knit-button.
...
We simulate a shystem with two identical lower-layer tree species, which only differ in their initial planting density:
...
STRANGELY: DESPITE THE HIGHER PLANTING DENSITY FOR TREE SPECIES 1, IT TENDS TO RETURN TO EQUAL VALUES OF GROUND COVER AS SPECIES 2, I.E. BOTH AT 50%. WHY ARE THEIR INITIAL PROPORTIONS NOT PRESERVED? MAYBE AN ERROR IN THE CALCULATION OF SAT(1:2)?

TODO:

1. Temporarily make SAT(1:3) output variables
2. Check whether the capping of SAT at a maximum of 1 (it can exceed that because of SHADEPROJ=2) is implemented correctly and consistently.
2a. For example, if before capping SAT(1)=1.2 And SAT(2)=1.0, then after capping both will end up with the same SAT and thus the same Ac, which is wrong.
2b. SAT-capping is done for the calculation of the initial state constants in CAF2021.f90, but it is not done in the daily loop. There, after calling 'call morphology(CBT_t,CST_t,LAIT_t,treedens_t, SAT_t)', no SAT capping is carried out. That should become apparent when we make SAT(1:3) output variables.
