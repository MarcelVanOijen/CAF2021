...
```{r}
  ...
  fileTM  <- "BC/BC_RESULTS_Turrialba-Masatepe/BC_2021-09-17_6e4/BC_Turrialba_Masatepe_06_45.RData"
  ...
  fileTM_MAP <- "BC/BC_RESULTS_Turrialba-Masatepe/BC_2021-09-17_6e4/CAF2021_parMAP_06_45.rds"
...
# ADDENDA after 2021-07-16

- Check literature to evaluate model including the six papers sent by Stefania Cerretelli, of which the last three were:
  - @Carpenter_2001_Landuse
  - @Castro-Tanzi_2012_Analysis
  - @Villatoro-Sanchez_2015_Temporal
...
Here we report on (1) assessing impacts of soil water holding capacity, (2) simulating fruit production, (3) simulating the SEACAF farms.
...
The work reported here builds on the earlier development and calibration of model CAF2021 which heavily depended on data from the long-term coffee agroforestry experiments in Turrialba (Costa Rica) and Masatepe (Nicaragua). The experiments will be referred to at various places in this report, so we recall some key results in this section. There were a total of 32 treatments in the experiments, and we show observed vs. simulated average annual coffee dry matter yields for all 32 treatments in Fig. \ref{fig:yObsSimTM}. Various model outputs for the 32 experimental treatments are shown in Fig. \ref{fig:TM}. For simulation of other variables, see the previous Progress Report [@VanOijen_2021_Modellinga]. The simulation results shown in Figures \ref{fig:yObsSimTM} and \ref{fig:TM} were derived with the Maximum A Posteriori (MAP) parameter vector of the model, determined by Bayesian calibration of the model against the experimental data.

No further model calibration has taken place apart from minor adjustment of parameter settings for fruit production as we discuss further below. All results shown in this report are from model runs using the MAP parameter vector. The focus is not on general parameters but on site-specific settings for weather, soil conditions, shading and management.
...
```{r, fig.height=3, fig.cap="\\label{fig:yObsSimTM}Observed vs. MAP-simulated yields for the 32 treatments in Turrialba (black numbers) and Masatepe (red). Left: Turrialba (n=18). Middle: Masatepe (n=14). Right: both locations (n=32)."}
...
```{r TM, fig.height=8, fig.cap="\\label{fig:TM}Summary results of simulations for Turrialba (grey bars) and Masatepe (red)."}
...
Fig. \ref{fig:yObsSimTMwater} shows simulated vs. observed yields for the Turrialba and Masatepe experiments after increasing the soil water holding capacity (difference between field capcity and wilting point). This change was established by decreasing the water level at which wilting point is reached. When we compare the results of Fig. \ref{fig:yObsSimTMwater} with those of the original simulations (Fig. \ref{fig:yObsSimTM}), we see that the results for Turrialba differed very little but simulated yields for Masatepe increased considerably. This reflects the fact that the conditions in Nicaragua were more drought-prone to begin with.

```{r, fig.height=3, fig.cap="\\label{fig:yObsSimTMwater}Observed vs. simulated yields for the 32 treatments in Turrialba (black numbers) and Masatepe (red). Same figure as above but for simulations with higher soil water holding capacity (FWCWP reduced from 0.47 to 0.20)."}
  source('initialisation/initialise_CAF2021_general.R')
  par( mfrow=c(1,1), mar=c(2,2,2,2), pty="s" )
...
In this section, we study the impact of adding banana plants on various outputs from the model. The reference simulations are those that we looked at above, i.e. those for the long-term experiments in Turrialba and Masatepe which did not include any fruit trees.
...
The key literature information used for parameterising banana was the following:

- @Damour_2012_Simulation: SLA of 0.018 $m^2 g^{-1}$, light extinction coefficient = 0.7, leaf N-content 3-4% of dry matter.
- @ChavesC._2009_Modeling: plantain (Musa AAB). Latin American mean yield is 7.3 t ha-1 y-1. DMP fruits ~ 25%. LUE = 1.63 g MJ-1. Critical temperatures of $T_{min}$ = 12.5 $^{\circ}$C, $T_{opt}$ = 25 $^{\circ}$C, $T_{max}$ = 40 $^{\circ}$C. Allocation about equally to leaves, pseudostem and corm with during later development most going to bunches that ultimately reach up to 45% of aboveground biomass.
...
- @Ripoche_2012_Modeling: Up to 10 kg aboveground DM per plant.
...
To test the impacts on ecosystem services of including banana in coffee agroforestry systems,
we now virtually add 100 banana plants per hectare to each of the experimental treatments in Turrialba and Masatepe that we investigated above (Fig. \ref{fig:TM}). We show the *differences* from those earlier results in Fig. \ref{fig:TM.B}. Overall, the impacts are fairly small. There is some reduction in coffee production, especially in the originally shadeless treatments (numbers 17 and 18) at Turrialba, and in timber production with Chloroleucon (treatments 13-16). The addition of banana plants led to some extra storage of carbon in the system in all four shadeless treatments (17-18 in Turrialba, 19-20 in Masatepe).
...
```{r TM.B, fig.height=8, fig.cap="\\label{fig:TM.B}Differences between simulations for Turrialba (grey bars) and Masatepe (red) with 100 banana plants per hectare added and default simulations without fruit plants."}
  ...
  barplotTM( dfOutTM.B[,"D_Nsoil"], name="Soil N (kg ha-1 y-1)", xtxt=1:32 )
```
...
A similar exercise as for banana was carried out for avocado.
...
- @Elzebroek_2008_Guide: optimal temperature (three races): day 20-35C, night 10-25C. Cultivar Hass is on the cold side, so we estimate optima as day 25C, night 15C, 24-h optimum 20C. First harvests after 4 years (vegetatively propagated) to 5-6 yrs (seedlings).
- @Monzon-Martinez_2019_Perfil: harvest seasons of avocado in Guatemala: July-Sep & Nov-April (so 9 months in total, i.e. all months except May-June & Oct). Yields in different departments from 4-11.5 t ha-1 y-1, mean 7 t ha-1 y-1.
...
We now virtually add 100 avocado trees per hectare to each of the experimental treatments in Turrialba and Masatepe. The new results are shown in Fig. \ref{fig:TM.A}, and we again compare these with the results for the simulations for the experimental conditions without fruit trees (Fig. \ref{fig:TM}). The impact of avocado on other ecosystem services is only very slightly higher than that of banana.
...
```{r TM.A, fig.height=8, fig.cap="\\label{fig:TM.A}Differences between simulations for Turrialba (grey bars) and Masatepe (red) with 100 avocado trees per hectare added and default simulations without fruit trees."}
  ...
  barplotTM( dfOutTM.A[,"D_Nsoil"], name="Soil N (kg ha-1 y-1)", xtxt=1:32 )
```
...
Data on field slopes at the farm locations were taken from SEACAF-file **slope_fincas.xlsx**. The slopes were calculated using GRASS-GIS by Stefania Cerretelli (Univ. Greenwich) from 1-arc-second digital elevation data delivered by NASA's SRTM mission (https://www.usgs.gov/science-support/osqi/yes/resources-teachers/earth-explorer).
...
Data on farm properties and yields were taken from SEACAF-file **typology.XLSX**. Some of the data required post-processing to make them suitable for use with CAF2021. The following variables were taken from the file:

- *kg_ha* which is fresh coffee bean yield, which we divided by 3 for conversion to dry weight.
- *spec_trees* which is the numnber of shade tree species.
- *VisSky* which is the fraction of visible sky from which we calculated shade level as 1-VisSky.
- *kg N/ha* which is the annual rate of N-fertilisation.
...
Soil information was taken from the SEACAF-file **Soil_general.xlsx** provided by Lucie Büchi (Univ. Greenwich). The data required some post-processing for use with CAF2021:

- The data in this file refer to the top 26 cm of the soil whereas CAF2021 requires data for the whole rooted zone. For the necessary conversion, we made the assumption that the top 26 cm. contains two-thirds of the soil organic matter.
...
A summary of the soil data derived this way is shown in the following table.
...
```{r XX8, fig.height=8, fig.cap="\\label{fig:XX8}Data derived from the SEACAF inventory for farms in Costa Rica (black) and Guatemala (red), plotted against altitude. The bottom four panels show soil variables: density (DM; g dry soil cm-3), carbon-nitrogen ratio (C/N; -), carbon and nitrogen content of the rooted zone (C, N; kg m-2)."}
...
We combined all data for simulation of the farms (both agricultural and weather variables) in one large data structure (R data-frame) which facilitated checking data consistency. Fig. \ref{fig:XX8} shows data values for all farms for eight different variables, all plotted against altitude. Correlations with altitude are mostly weak, with only some posive correlation between altitude and coffee yield.

Some values of N-fertilisation rate seem implausibly high. 

We would expect to find some correlation between number of shade tree species, the level of shade that they cause, fertilisation rate and yield, but generally these four variables are only weakly correlated as shown in Fig. \ref{fig:XX10} (where we capped N-fert at 500 kg N ha-1 y-1). Only yield and fertilisation show some positive correlation.

```{r XX10, fig.height=8, fig.cap="\\label{fig:XX10}Correlations within the SEACAF database. Top two rows: Costa Rica. Bottom two rows: Guatemala. The 1st and 3rd row show how yield is correlated with number of shade trees (nt; -), shade level (-), N-fertilisation (Nfert; kg N ha-1 y-1). The 2nd and 4th row show correlations amongst those environmental variables."}
  ...
  x=nt    ; y=yield ; plot(x,y,xlab="nt"   ,ylab="yield", col="red") ; add.lmleg()
  x=shade ; y=yield ; plot(x,y,xlab="shade",ylab="yield", col="red") ; add.lmleg()
  x=Nfert ; y=yield ; plot(x,y,xlab="Nfert",ylab="yield", col="red") ; add.lmleg()
  
  x=nt    ; y=shade ; plot(x,y,xlab="nt"   ,ylab="shade", col="red") ; add.lmleg()
  x=nt    ; y=Nfert ; plot(x,y,xlab="nt"   ,ylab="Nfert", col="red") ; add.lmleg()
  x=shade ; y=Nfert ; plot(x,y,xlab="shade",ylab="Nfert", col="red") ; add.lmleg()
```
...
```{r, eval=F}
  ...
  for(fi in 1:n_C.f) {
    ...
    for(s in 1:32) {
      ...
      params <- parMAP[,s]
      outTM  <- run_model( params, matrix_weather_fi, calendar_fert,
...
## Evaluating the WorldClim weather files for use at farm locations
...
To assess the impact of weather and management in CAF2021, we ran multiple simulations for all 179 farms, where the only difference between farms was their weather. For each farm, we ran all 32 experimental treatments (fertilisation, shading) that were applied in Turrialba and Masatepe. The simulated yields of these 179 x 32 simulations are summarized in Fig. \ref{fig:XX12}. The figure comfirms that the model is responsive to both management and weather. The management effects are seen in the range of values for each individual farm: the 32 multi-annual average yields varied strongly from close to zero up to 2000-3000 kg ha-1 y-1 for most farms. The importance of weather can be assessed from the yield-differences between farms. Because weather is related to altitude, a significant fraction of mean yield variation was accounted for by altitude (r2 = 0.45 in Costa Rica, r2=0.53 in Guatemala).

```{r XX12, fig.height=3.5, fig.cap="\\label{fig:XX12}Simulations of coffee yield for farms in Costa Rica (left) and Guatemala (right). For each farm 32 simulations were carried out using the conditions for Turrialba and Masatepe. The only farm-specific information used was their WorldClim weather. Filled black dots: average of the 32 yields. Circles: maximum and minimum yields."}
  ymean_C.f_sim <- dfoutTMSummary( df_C.f_sim )[["mean"]][,"harvDM_f_hay"]
  ymean_G.f_sim <- dfoutTMSummary( df_G.f_sim )[["mean"]][,"harvDM_f_hay"]
  ymin_C.f_sim  <- dfoutTMSummary( df_C.f_sim )[["min" ]][,"harvDM_f_hay"]
  ymin_G.f_sim  <- dfoutTMSummary( df_G.f_sim )[["min" ]][,"harvDM_f_hay"]
  ymax_C.f_sim  <- dfoutTMSummary( df_C.f_sim )[["max" ]][,"harvDM_f_hay"]
  ymax_G.f_sim  <- dfoutTMSummary( df_G.f_sim )[["max" ]][,"harvDM_f_hay"]

  par( mfrow=c(1,2), mar=c(4,2,3,2), mgp=c(2,1,0) )
  
  ymax <- max( ymax_C.f_sim, ymax_G.f_sim )
  
  plot  ( df_C.f$altitude, ymean_C.f_sim,
          main="Simulated yield\n(Costa Rica)",
          xlab="Alt. (m)", ylab="", ylim=c(0,ymax), pch=20 )
  points( df_C.f$altitude, ymax_C.f_sim, pch=1, cex=0.5 )
  points( df_C.f$altitude, ymin_C.f_sim, pch=1, cex=0.5 )
  y.lm <- lm( ymean_C.f_sim ~ df_C.f$altitude )
  r2   <- round( summary(y.lm)$r.squared, 2 )
  abline( y.lm$coefficients[1], y.lm$coefficients[2], col="blue" )
  legend( "topright",
          legend=paste("r2= ",as.character(r2)),
  ...
  plot  ( df_G.f$altitude, ymean_G.f_sim,
          main="Simulated yield\n(Guatemala)",
          xlab="Alt. (m)", ylab="", ylim=c(0,ymax), pch=20 )
  points( df_G.f$altitude, ymax_G.f_sim, pch=1, cex=0.5 )
  points( df_G.f$altitude, ymin_G.f_sim, pch=1, cex=0.5 )
  y.lm <- lm( ymean_G.f_sim ~ df_G.f$altitude )
  r2   <- round( summary(y.lm)$r.squared, 2 )
  abline( y.lm$coefficients[1], y.lm$coefficients[2], col="blue" )
  legend( "topright",
          legend=paste("r2= ",as.character(r2)),
...
```{r}
  r2_T_weatherC <- r2_M_weatherC <- rep( NA, n_C.f )
  r2_T_weatherG <- r2_M_weatherG <- rep( NA, n_G.f )

  for(f in 1:n_C.f) {
    y_TM.sim_C       <- df_C.f_sim$outTM[[f]] [,"harvDM_f_hay"]
    yT_Cf.lm         <- lm( y_TM.obs[iT] ~ y_TM.sim_C[iT] )
    yM_Cf.lm         <- lm( y_TM.obs[iM] ~ y_TM.sim_C[iM] )
    r2_T_weatherC[f] <- summary(yT_Cf.lm )$r.squared
    r2_M_weatherC[f] <- summary(yM_Cf.lm )$r.squared
  }

  for(f in 1:n_G.f) {
    y_TM.sim_G       <- df_G.f_sim$outTM[[f]] [,"harvDM_f_hay"]
    yT_Gf.lm         <- lm( y_TM.obs[iT] ~ y_TM.sim_G[iT] )
    yM_Gf.lm         <- lm( y_TM.obs[iM] ~ y_TM.sim_G[iM] )
    r2_T_weatherG[f] <- summary(yT_Gf.lm )$r.squared
    r2_M_weatherG[f] <- summary(yM_Gf.lm )$r.squared
  }
```

These simulations of farm yields using farm-specific weather but management and shading from the Turrialba and Masatepe experiments also allowed us to assess the quality of the weather data that we derived from WorldClim. Because each farm-weather was combined with the 18 Turrialba treatments and the 14 Masatepe treatments, we could calculate how well the observed experimental yields were still accounted for by application of the wrong (farm's) weather instead of the correct experiment weather. With the correct experimental weather, the calibrated CAF2021 model accounted for 75% of observed yield-variation in the Turrialba experiment and also 75% of variation in Masatepe (see the reference simulations of Fig. \ref{fig:yObsSimTM}). With incorrect weather from any of the farms, r2 was always lower as shown in Fig. \ref{fig:XX13}. However, the degree to which the experiment simulations deteriorated when using farm weather varied widely, and in particular the Turrialba experiment could be simulated almost equally well with the weather from some farms. Closer inspection of the yield-simulations for Turrialba showed that these farms were all close to Turrialba, no more than 20 km away (bottom panel of Fig. \ref{fig:yObsSimTM}). This inspires confidence in the use of WorldClim weather data as input for CAF2021.

```{r XX13, fig.height=6, fig.cap="\\label{fig:XX13}Simulating the Turrialba and Masatepe experiments: impact of replacing the weather at the experiments with that from farms in Costa Rica (black) and Guatemala (red). The dashed blue lines indicate the fraction of variance accounted for by the original CAF2021 modelling (r2) when the correct experiment weather was used. Top panel: r2 for Turrialba yield-simulations (n=18) using farm weather, plotted against farm index. Middle panel: same but for Masatepe (n=14). Bottom panel: r2 for Turrialba yield-simulations using Costa Rican farm weather plotted against distance between farm and experiment."}
  par( mfrow=c(3,1), mar=c(4,2,2,2), pty="m", mgp=c(2,1,0) )

  plot  ( 1:n_G.f, r2_T_weatherG, main="r2 for yields in Turrialba simulated with weather from farms in C.R. and G.",
          col="red", xlab="Farm index", ylab="", ylim=c(0,1.15) )
  points( 1:n_C.f, r2_T_weatherC )
  ...
  plot  ( 1:n_G.f, r2_M_weatherG, main="r2 for yields in Masatepe simulated with weather from farms in C.R. and G.",
          col="red", xlab="Farm index", ylab="", ylim=c(0,1.15) )
  points( 1:n_C.f, r2_M_weatherC )
  ...
  plot  ( dist_C.T, r2_T_weatherC, main="r2 for yields in Turrialba simulated with weather from farms in C.R.",
...
```{r}
  ...
  varnames <- c( "code","slope","nt","Nfert","y.obs",
  ...
  colnames(listmat_nt_C[[1]]) <- varnames
  ...
  colnames(listmat_nt_C[[2]]) <- varnames
  ...
  colnames(listmat_nt_C[[3]]) <- varnames
  ...
  colnames(listmat_nt_G[[1]]) <- varnames
  ...
  colnames(listmat_nt_G[[2]]) <- varnames
  ...
  colnames(listmat_nt_G[[3]]) <- varnames
```
...
```{r XX16, fig.height=5, fig.cap="\\label{fig:XX16}Yield-observations for farms in Guatemala plus simulations of five variables: see previous figure for explanation of variables."}
  par(mfrow=c(2,3),mar=c(4,2,2,2), mgp=c(2,1,0))
...
```{r, eval=F}
  ## Selecting farm-specific TM-treatments

  # iTM_s0_N0 <- 18 ; iTM_s0_N1 <- 17 ; iTM_s1_N0 <- 14 ; iTM_s1_N1 <- 13
  iTM_s0_N0 <- 20 ; iTM_s0_N1 <- 19 ; iTM_s1_N0 <- 30 ; iTM_s1_N1 <- 29

  TM_C.f <- rep( NA, n_C.f ) ; TM_G.f <- rep( NA, n_G.f )
  
  Nthr <- 175
  for( fi in 1:n_C.f ) {
    if( (df_C.f$nt[fi]<=2) && (df_C.f$Nfert[fi]< Nthr) ) TM_C.f[fi] <- iTM_s0_N0
    if( (df_C.f$nt[fi]<=2) && (df_C.f$Nfert[fi]>=Nthr) ) TM_C.f[fi] <- iTM_s0_N1
    if( (df_C.f$nt[fi]> 2) && (df_C.f$Nfert[fi]< Nthr) ) TM_C.f[fi] <- iTM_s1_N0
    if( (df_C.f$nt[fi]> 2) && (df_C.f$Nfert[fi]>=Nthr) ) TM_C.f[fi] <- iTM_s1_N1
  }
  
  for( fi in 1:n_G.f ) {
    if( (df_G.f$nt[fi]<=2) && (df_G.f$Nfert[fi]< Nthr) ) TM_G.f[fi] <- iTM_s0_N0
    if( (df_G.f$nt[fi]<=2) && (df_G.f$Nfert[fi]>=Nthr) ) TM_G.f[fi] <- iTM_s0_N1
    if( (df_G.f$nt[fi]> 2) && (df_G.f$Nfert[fi]< Nthr) ) TM_G.f[fi] <- iTM_s1_N0
    if( (df_G.f$nt[fi]> 2) && (df_G.f$Nfert[fi]>=Nthr) ) TM_G.f[fi] <- iTM_s1_N1
  }
...
The results look plausible as far as the difference between Turrialba (not responsive to increased water availability) and Masatepe (responsive) is concerned. More drought analysis could be carried out, although this is likely of more interest to farms in drier areas such as parts of Nicaragua which are not included in SEACAF.

## Simulating fruit production

Due to lack of reliable data, no rigorous Bayesian calibration for fruit production parameters was carried out. Banana production data from farms in the Turrialba region of Costa Rica varied over orders of magnitude and could thus not be used. However, the literature information seemed enough to allow the preliminary modelling of the effects of fruit trees in coffee agroforestry systems on ecosystem services that we reported here.

## Input data for CAF2021

### SEACAF database

The SEACAF database is rich and impressive. Some information is lacking, including on shade management, properties of soil layers below 28 cm, and uncertainties associated with the data.

### Weather data

Besides WorldClim data, information from weather stations in both countries is available to the project. However, there are gaps and errors in these data and using them instead of WorldClim to produce CAF2021 daily weather data files for all five variables and all 179 farms locations would constitute a major effort. Results shown in this report suggest that the simpler option of using the WorldClim data is sufficent for the purposes of the project. However, the WorldClim data end at 2018 and we may want to use weather station data to estimate anomalies for 2019 to be added to the long-term climatic averages from WorldClim.

## Simulating farms in Costa Rica and Guatemala

### Preceding model calibration

The model was previously calibrated on data from Turrialba and Masatepe, and no further calibration was carried out because no other reliable long-term data were available. This means that the model has not been calibrated using data from Guatemala, and not for any high-altitude location. 

### Modelling results

The figures shown in this report suggest that CAF2021 may be a useful 'discussion partner' in the assessment of ecosystem services associated with coffee agroforestry. The model helps analyse the roles of weather, soils and management. Various correlations of system behaviour with altitude and N-fertilisation rate were explored. The impact of fertilisation is shown to be both good (increasing yield and soil carbon, decreasing erosion) and bad (N-leaching). On the other hand, the model analysis found no clear effects of shading, but these were not found in any farm data regression analyses either.

### Caveats and possible improvements in data use and modelling

The status of the simulations reported here remains preliminary and tentative. The model has only been calibrated to very restricted data (two locations, neither at high altitude) and model outputs have not been carefully compared against data.

- The comparisons of observed with simulated yields reported here may be misleading as the data were from the single year 2019 and the model results were averages over 2000-2014. Harmonisation will require extending the WorldClim weather data files (which run to 2018) by a year using information from local weather stations in both countries.
- Some high yields observed on farms were not matched by the model. One possible explanation for this may be that the model does not allow each year's harvesting season to extend beyond December into the next calendar year. Before changing the model in this respect, we need to analyse the data to check whether observed high yields were indeed associated with slow-maturing coffee in cool high-altitude locations.
- We also made no explicit comparison of simulated against observed shade levels, but overall the simulated shade levels seem much too high.
- Some of the shade overestimation may be due to the model using Inga management regimes that were used in Masatepe.
- The quality of modelling of other ecosystem services has not been evaluated yet. There is limited information from the farms themselves, so a more general comparison with the literature will be needed, e.g. publications on soil processes including erosion in Central America [@Ataroff_1997_Soil; @BlancoSepulveda_2015_Soila; @Cannavo_2011_Agroforestry ; @Craswell_1998_Agroforestry].

## Further work

The amount of future modelling work that can still be carried out wihtin the short SEACAF projected is severely time-limited. There are only two project-months left and the last one is reserved for model documentation, training and publication. We therefore suggest the following program of work for the next month:

- No further development of model structure. Leave to a future project the implementation of extended coffee bean maturation beyond the end of the calendar year.
- No further time spent on data acquisition and Bayesian calibration of the model.
- But some minor additional data cleaning will be necessary. For example, correction of the impossible nitrogen fertilisation levels at two farms in Guatemala.
- We need to make more use of farm-specific data as inputs for modelling. This includes the use of information on shade tree species and their management (thinning, pruning), if available.
- The model is currently driven by external thinning and pruning calendars. We may need to move to simulating shade-level goal-directed management to allow the model to reduce shade levels were appropriate. Alternatively, we may keep the model structure as it is but set the shade management calendars to higher pruning frequency and intensity for some of the farms.
- For model evaluation of variables not measured in the project, we need to compare model outputs to the published results on coffee agroforestry systems.
- The main focus of the remaining project time for modelling should be on application of SEACAF:
  - Extend simulations to include the farm inventory year 2019, which will require collecting farm weather station data for that year to be added as anomalies to the climatic trends.
  - Analyse impacts of environment vs. management (farm typology analysis).
  - Analyse interannual variability of yield and other ecosystem services. This includes investigation of the impacts of El Nino and La Nina weather anomalies on system performance. The impact of drought years in the model should be analysed in greater detail than done thus far (in the brief water holding capacity analysis presented in this report).
...
I thank the Natural Resources Institute of the University of Greenwich for funding, and my colleagues in project SEACAF and earlier projects for support. Lucie Büchi, Stefania Cerretelli, Jeremy Haggar, Erick René López de Paz and Alejandra Ospina provided advice on using data for farm simulations.
