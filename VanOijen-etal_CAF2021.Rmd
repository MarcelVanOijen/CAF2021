...
- The goal of sustainable agriculture puts multiple demands on coffee agroforestry systems (CAF). The productivity of coffee plants and shade trees must be high while negative impacts on the environment are minimised. In this paper, we refer to each form of productivity or environmental impact as an *ecosystem service* (ES). An important issue in the evaluation of CAF is that many ES are difficult to quantify, which hampers the choice of shade tree species and farm management.
- This is a particular problem in Central America, where mountainous topography with young volcanic soils leads to strong spatial variation in weather patterns and soil fertility. Together with a wide variation in management choices, this has led to strong variability in productivity. It likely also caused strong variation in environmental ES, but these have not been measured as much.
- Here, we used a combination of data analysis and process-based modelling to quantify ecosystem services at coffee agroforestry farms: 89 farms in Costa Rica and 79 in Guatemala. 
- The same farms were studied previously by Haggar et al. (2021), who showed ...
- Measurements at these farms did not include ES related to carbon and nitrogen biogeochemistry. So we estimated these ES using the new coffee agroforestry model CAF2021, which we introduce in this paper. The model differs from its predecessors CAF2007 and CAF2014 mainly in that it allows for more complex agroforestry systems with up to three different shade tree species.
- Other models for coffee: ...
- Details of the structure of CAF2021 are provided in the Materials and Methods section and online.
- To calibrate the model, we used multivariate data from 32 different treatments applied in long-term field experiments in Costa Rica and Nicaragua. A subset of these data (10 treatments) was used earlier in the Bayesian calibration of CAF2014, a model that could not simulate experimental treatments with multiple shade tree species. Bayesian calibration has become a common method for model calibration and uncertainty quantification of process-based models (vO 2020). Here we use the same method of Bayesian model calibration by means of Markov chain Monte Carlo sampling that was used by Ovalle et al.
...
- Farm inventory:
  ...
  - Soil data: C, N, texture information from which we infer water-holding capacity using a pedotransfer function. Assuming that bottom soil layers contain SOM equivalent to 20% of top-soil SOM.
...
- Erythrina + Inga: MAP parameter settings
...
- Parameter SA.
- Assessment of yield and ES stability after a severe drought.
...
- The model seems to be able to simulate a wide range of CAF systems. Despite much information being unavailable, and no farm-level model calibration, still good simulation of coffee yields in Costa Rica.
...
- Take a three-three system from C or G: Service-fruit-timber
