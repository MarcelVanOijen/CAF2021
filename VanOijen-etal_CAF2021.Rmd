...
- Here, we used a combination of data analysis and process-based modelling to quantify ES at coffee agroforestry farms: 89 farms in Costa Rica and 79 in Guatemala. 
...
- We summarise farm properties using the typology of Haggar et al. (2021).
- ENVIRONMENTAL TABLE: per country four farm types, so a table with 8 rows and as columns: n, averages of altitude, slope, temperature, rain, soil C, soil N, water-holding capacity
  - State that water-holding capcity was derived from soil texture information using the pedotransfer function tested by Tomasella et al. (). We further assume that bottom soil layers contain SOM equivalent to 20% of top-soil SOM. Weather data (5 variables, only two summarised in the table): WorldClim plus country-wide 2019 anomalies for rain and temperature.
- MANAGEMENT TABLE: as previous table but N-fertilisation and densities of 6 representative tree species.
