...
```{r}
  load( "outputNamesUnits.rda" )
  load( "outF.rda" )
  load( "df_CG.f_sim.rda" )
```
...
```{r Maps_CGold, eval=F, out.width="400px", fig.align="center", fig.cap="\\label{Maps_CGold}Altitude maps for Costa Rica and Guatemala with locations of the coffee farms."}
  knitr::include_graphics("Maps_CGold.png")
```

```{r Maps_CG, out.width="90%", fig.align="center", fig.cap="\\label{Maps_CG}Locations of sampled coffee farms in Costa Rica and Guatemala."}
  ...
  text( guate@polygons[[1]]@labpt[1]    , guate@polygons[[1]]@labpt[2],
        "GUATEMALA" , font=2, col="black" )
  text( costa@polygons[[1]]@labpt[1]-0.4, costa@polygons[[1]]@labpt[2]+0.5,
        "COSTA RICA", font=2, col="black" )
  text( pan@polygons[[1]]@labpt[1]-1    , pan@polygons[[1]]@labpt[2],
        pan@data$NAME_0 )
  text( nica@polygons[[1]]@labpt[1]     , nica@polygons[[1]]@labpt[2],
        nica@data$NAME_0 )
  text( hon@polygons[[1]]@labpt[1]      , hon@polygons[[1]]@labpt[2],
        hon@data$NAME_0 )
...
costaCol<-"indianred"	#LB
guateCol<-"steelblue3"	#LB  
par( mfrow=c(1,3), mar=c(2,4,2,2), pty="s" )
  ...
  plot( y_T[,"Y.sim"], y_T[,"Y.obs"],
        main="Turrialba, Masatepe",
        xlab="Simulated coffee yield (kg ha-1)",
        ylab="Observed coffee yield (kg ha-1)",
  ...
  legend( "bottomright",
          legend=c(paste("Turrialba: r2 = ",as.character(r2_T)),
                   paste("Masatepe: r2 = ",as.character(r2_M)) ),
          col=c("black","red"), lty=c(1,1), cex=0.7, bty="n")
  # mtext("A.",side=3,adj=0,line=0.4,font=2,cex=0.8)	#LB
  ...
  plot( sim_C, obs_C,
        ...
        cex.main=1, asp=1 , pch=16,col=costaCol)
  ...
  #        lty=1, cex=0.7, bty="n" )
  mtext(paste0("r2 = ",r2_C),side=3,at=0,adj=0,line=-1.1,cex=0.7)	#LB
  # mtext("B.",side=3,adj=0,line=0.4,font=2,cex=0.8)	#LB
  ...
  plot( sim_G, obs_G,
        ...
        cex.main=1, asp=1 , pch=15,cex=0.9,col=guateCol)
  ...
  #        lty=1, cex=0.7, bty="n" )
  mtext(paste0("r2 = ",r2_G),side=3,at=0,adj=0,line=-1.1,cex=0.7)	#LB
  # mtext("C.",side=3,adj=0,line=0.4,font=2,cex=0.8)	#LB  		  
```
...
```{r}
  plotimpacts_CG <- function( FC=oF1_C, FG=oF1_G, xvar="Nfert",
    ...
    plot  ( xC, yC,
      ...
      main=title,
      pch=16,cex=0.8,
      xlim=c( min(xC,xG        ), 1.2*max(xC,xG        )),
      ylim=c( min(yC,yG,na.rm=T), 1.2*max(yC,yG,na.rm=T)), col=costaCol, ... )
    points( xG, yG, pch=15, cex=0.7,col=guateCol  )
    mtext(npanel,side=3,adj=0,line=0.4,font=2,cex=0.8)
    y.lm_C <- lm( yC ~ xC ) ; y.lm_G <- lm( yG ~ xG )
    r2_C   <- round( summary(y.lm_C)$r.squared, 2 )
    r2_G   <- round( summary(y.lm_G)$r.squared, 2 )
    abline( y.lm_C$coefficients[1], y.lm_C$coefficients[2], col=costaCol )
    abline( y.lm_G$coefficients[1], y.lm_G$coefficients[2], col=guateCol )
    legend( legpos,
            legend=c( paste0("r2=",as.character(r2_C)),
                      paste0("r2=",as.character(r2_G)) ),
            col=c(costaCol,guateCol), lty=c(1),
            pch=c(16,15),pt.cex=c(0.8,0.7),cex=0.7,
            bg="transparent" ,bty="n")
  }
```

```{r FarmSimNfert, fig.height=4.5, fig.cap="\\label{fig:FarmSimNfert}Simulations of ecosystem services at farms in Costa Rica (black) and Guatemala (red). Units as in Table 3. All plots are against N-fertilisation level (kg N ha-1 y-1)."}
  old.par <- par(no.readonly = TRUE)

  par( mfrow=c(2,4), mar=c(3,2,1,1), mgp=c(2,1,0), oma=c(4,2,2,2))

  yv <- "harvDM_f_ha1819" ; yv2 <- "harvDM_f_ha1920"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="Nfert", yvar=yv, yvar2=yv2,
                   legpos="top"     , title="Coffee yield",
                  xleg="", npanel="")
  yv <- "harvDMST_3_hay"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="Nfert", yvar=yv,
                   legpos="topright", title="Timber" ,
                  xleg="", npanel="")
  yv <- "harvCPT_f_hay"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="Nfert", yvar=yv,
                   legpos="topleft" , title="Fruit" ,
                  xleg="", npanel="")
  yv <- "D_Csoil"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="Nfert", yvar=yv,
                   legpos="topright",title="C-change" ,
                  xleg="", npanel="")
  yv <- "Crunoff_fb"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="Nfert", yvar=yv,
                   legpos="topright", title="C-runoff" ,
                  xleg="N fert. (kg N ha-1 y-1)", npanel="")
  yv <- "NfixT_fb"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="Nfert", yvar=yv,
                   legpos="topright", title="N-fixation" ,
                  xleg="N fert. (kg N ha-1 y-1)", npanel="")
  yv <- "Nleaching_fb"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="Nfert", yvar=yv,
                   legpos="topleft" , title="N-leaching" ,
                  xleg="N fert. (kg N ha-1 y-1)", npanel="")
  yv <- "Nemission_fb"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="Nfert", yvar=yv,
                   legpos="topleft" , title="N-emission" ,
                  xleg="N fert. (kg N ha-1 y-1)", npanel="")
  
  #this is to create the legend below the plots
  par(fig = c(0, 1, 0, 1), oma = c(0, 0, 0, 0), mar = c(0, 0, 0, 0), new = TRUE)
  plot(0, 0, type = "n", bty = "n", xaxt = "n", yaxt = "n")
  legend( "bottom", c("Costa Rica","Guatemala"), col=c(costaCol,guateCol),
          pch=c(16,15),lwd=2,pt.bg=c(costaCol,guateCol), cex=1,
          pt.cex=c(1.5,1.4), xpd=TRUE, horiz=TRUE, inset=0.03)
  par( old.par )
```

```{r FarmSimShade, fig.height=4.5, fig.cap="\\label{fig:FarmSimShade}Simulations of ecosystem services at farms in Costa Rica (black) and Guatemala (red). Units as in Table 3. All plots are against shade level (-)."}
  old.par <- par(no.readonly = TRUE)
  ...
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="shade", yvar=yv, yvar2=yv2,
                   legpos="topright", title="Coffee yield",
                  xleg="", npanel="")
  yv <- "harvDMST_3_hay"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="shade", yvar=yv,
                   legpos="topleft" , title="Timber" ,
                  xleg="", npanel="")
  yv <- "harvCPT_f_hay"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="shade", yvar=yv,
                   legpos="topright", title="Fruit" ,
                  xleg="", npanel="")
  yv <- "D_Csoil"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="shade", yvar=yv,
                   legpos="topleft" , title="C-change" ,
                  xleg="", npanel="")
  yv <- "Crunoff_fb"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="shade", yvar=yv,
                   legpos="topleft" , title="C-runoff" ,
                  xleg="Shade level (-)", npanel="")
  yv <- "NfixT_fb"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="shade", yvar=yv,
                   legpos="topleft" , title="N-fixation" ,
                  xleg="Shade level (-)", npanel="")
  yv <- "Nleaching_fb"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="shade", yvar=yv,
                   legpos="topright", title="N-leaching" ,
                  xleg="Shade level (-)", npanel="")
  yv <- "Nemission_fb"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="shade", yvar=yv,
                   legpos="topright", title="N-emission" ,
                  xleg="Shade level (-)", npanel="")
...
```{r FarmSimShadeold, eval=F, fig.height=3.5, fig.cap="\\label{fig:FarmSimShadeold}Simulations of ecosystem services at farms in Costa Rica (black) and Guatemala (red). All plots are against level of shading (-)."}
  par( mfrow=c(2,4), mar=c(4,2,2,1), mgp=c(2,1,0) )

  yv <- "harvDM_f_ha1819" ; yv2 <- "harvDM_f_ha1920"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="shade", yvar=yv, yvar2=yv2,
                   legpos="topleft", title="Coffee yield", xleg="Shading level", npanel="A.")
  yv <- "harvDMST_3_hay"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="shade", yvar=yv,
                   legpos="topleft"    , title="Timber" , xleg="Shading level", npanel="B.")
  yv <- "harvCPT_f_hay"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="shade", yvar=yv,
                   legpos="topright"   , title="Fruit" , xleg="Shading level", npanel="C.")
  yv <- "D_Csoil"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="shade", yvar=yv,
                   legpos="bottomright",title="C-change" , xleg="Shading level", npanel="D.")
  yv <- "Crunoff_fb"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="shade", yvar=yv,
                   legpos="topleft"    , title="C-runoff" , xleg="Shading level", npanel="E.")
  yv <- "NfixT_fb"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="shade", yvar=yv,
                   legpos="topleft"    , title="N-fixation" , xleg="Shading level", npanel="F.")
  yv <- "Nleaching_fb"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="shade", yvar=yv,
                   legpos="topright"   , title="N-leaching" , xleg="Shading level", npanel="G.")
  yv <- "Nemission_fb"
  plotimpacts_CG( FC=oF1_C, FG=oF1_G, xvar="shade", yvar=yv,
                   legpos="topright"   , title="N-emission" , xleg="Shading level", npanel="H.")
```
...
```{r dES, fig.height=4, fig.align="center", out.width="80%", fig.cap="\\label{fig:dES}Changes in ecosystem services when fertilisation or shading is halved. Units as in Table 3. All changes are normalised to the country average."}
  #par( mfrow=c(2,1), mar=c(4,2,4,2) )
  #names.dES <- c( 1:8, "", "", 1:8  )

  noms<-c("Coffee yield","Timber","Fruit","C-change","C-runoff","N-fixation","N-leaching","N-emission")
  par(mfrow=c(1,4),mar=c(1,1,1,1),oma=c(2,8,3,0))
  ss<-barplot(rev(doF3_C.m),horiz=T,beside=T,space=0,col=costaCol,xlim=range(c(doF3_C.m,doF3_G.m)))
  mtext(rev(noms),side=2,at=ss,line=0.5,las=1,font=2)
  mtext("Costa Rica",side=3,line=-0.7,font=2)
  mtext("50% fertilisation",side=3,at=0.25,line=0,font=2,outer=T)
  barplot(rev(doF3_G.m),horiz=T,beside=T,space=0,col=guateCol,xlim=range(c(doF3_C.m,doF3_G.m)))
  mtext("Guatemala",side=3,line=-0.7,font=2)
  barplot(rev(doF4_C.m),horiz=T,beside=T,space=0,col=costaCol,xlim=range(c(doF4_C.m,doF4_G.m)))
  mtext("Costa Rica",side=3,line=-0.7,font=2)
  mtext("50% shading",side=3,at=0.75,line=0,font=2,outer=T)
  barplot(rev(doF4_G.m),horiz=T,beside=T,space=0,col=guateCol,xlim=range(c(doF4_C.m,doF4_G.m)))
  mtext("Guatemala",side=3,line=-0.7,font=2)
...
Except for farm type 4.3, the observed and simulated coffee yields were higher in Costa Rica than in Guatemala, which matches the about twice as high fertilisation rates in Costa Rica (Tables 2 and 3). The simulations suggest that the higher fertilisation in Costa Rica also stimulated carbon sequestration, nitrogen leaching and emission (Fig. \ref{fig:FarmSimShade}). A positive effect of fertilisation on carbon sequestration was also shown by @Korschens_2021_LongTerm, in their review of long-term experiments worldwide and in several empirical studies [@Virto_2012_Carbon; @Triberti_2016_Longterm].\
    Other differences between the countries, such as production of more fruit but less timber in Costa Rica, were the result of different choices of shade tree species and planting density rather than differential impacts of fertilisation on tree species (Tables 2 and 3).\
    The model points to inevitable trade-offs between productivity of coffee, productivity of trees and environmental ecosystem services. Changing the level of fertilisation or shading can benefit some ecosystem services but always to the detriment of others. This was apparent from the farm simulations shown in Figures \ref{fig:FarmSimNfert} and \ref{fig:FarmSimShade}, but was highlighted most clearly in the sensitivity analysis presented in Fig. \ref{fig:dES}. The benefits from fertilisation for yield and carbon sequestration must be balanced against the costs associated with increased nitrogen leaching and emission. Likewise the benefits for coffee yield of removing shading must be balanced against the associated losses in tree production (fruit, timber) and carbon sequestration as well as the reduced protection against erosion. Shading had only a minor impact on carbon sequestration in the simulations (Fig. \ref{fig:dES}), and the same was recently reported for the Turrialba experiment by @Sauvadet_2019_Shade. Protection against erosion by high canopy cover, as simulated here, has repeatedly been observed in coffee agroforestry systems [@Meylan_2017_Evaluating; @DeLeijster_2021_Ecosystem and references therein].\
...
```{r BC_T1, out.width="90%", fig.align="center", fig.cap="\\label{BC_T1}Calibration results for treatment 1 of the Turrialba experiment. Blue points with error bars: data. Black lines: CAF2021 simulation using the MAP parameter vector. Green lines: simulation using the maximum likelihood parameter vector. For this treatment of highly fertilised coffee shaded by Erythrina poeppigiana trees, seven variables were measured. From top-left and row by row: coffee yield, shade, tree crown area and height, aboveground carbon in coffee and trees, soil carbon gain."}
  file_T1.png <- "BC_outputs_data_01.png"
