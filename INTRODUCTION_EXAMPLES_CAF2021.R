#                       CAF2021: INTRODUCTION AND EXAMPLES
#                          Marcel van Oijen, 2021-11-23
...
# We initialise the model for a coffee agroforestry system with one shade tree
# species (Erythrina poeppigiana) in Turrialba (Costa Rica) 2000-2017:
...
# listed in the vector 'outputNames':
...
# with each other.
  ...
  source("initialisation/initialise_CAF2021_Turrialba_1.R")
  output_shade   <- run_model()
  source("initialisation/initialise_CAF2021_Turrialba_2.R")
...
#   'initialisation/initialisation_CAF2021_general.txt'
#   'model/CAF2021.f90'.
...
# 'compile_CAF2021.bat'. That will create a new 'CAF2021.DLL'.
...
# file 'compile_CAF2021.bat' to accomplish that.
...
#   'initialisation/initialise_CAF2021_Turrialba_1' and
#   'initialisation/initialise_CAF2021_Turrialba_2'.
...
# gives the same results as above), is as follows:
  ...
  SA( parname_SA = "KEXT",
      ...
      file_init  = "initialisation/initialise_CAF2021_Turrialba_1.R" )
...
# file 'in file 'BC/BC_CAF2021_MCMC_init_Turrialba_1').

  source("BC_CAF2021_Turrialba_1.R")

# The results of the BC are written to four files:
# 1. 'CAF2021_parModes_[].txt': parameter values
...
# Multi-site BC can be done as in the following example:
  ...
  source("BC_CAF2021_Turrialba_1&2.R")
...
# the day-number, to determine which file is the most recent one!]
  ...
  files_parModes <- list.files( pattern="CAF2021_parModes" )
  file_parModes  <- tail( files_parModes, 1 )

# Now we do the SA, starting from the MAP parameter vector

  SA_BC( "KEXT", file_init_BC = "BC/BC_CAF2021_MCMC_init_Turrialba_1&2.R",
...
################################################################################
### 9. Single run of CAF2021 using a parameter vector from previous BC
################################################################################
...
# ways of doing that. The file 'run_CAF2021_BC_parMode.R' shows two.
...
# 'run_CAF2021_Turrialba_1_AdditionalProcessingOutput.R'.
...
################################################################################
# CAF2021 has two parameters that regulate coffee bean sink strength that both
...
################################################################################
# CAF2021 simulates flowering in a slightly different way from CAF2007.
