...
```{r list_outF_C, cache=T}
  ...
  for(fi in 1:n_C.f) {
    ...
    # params <- set_par( "CSOM0MULT"      , 4/5 )
    # params <- set_par( "HMAX(3)"        , 50  )
    # ip     <- which( names_params=="KACEXP(3)" ) ; params[ip] <- params[ip] * 1.2
...
```{r list_outF_C_SA, eval=T, cache=T}
...
```{r list_outF_G_SA, eval=T, cache=T}
