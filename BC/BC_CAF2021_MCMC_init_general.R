...
#	 mult_sd_low <- 0.1
	 ###########################################################################
#     for (i in 1:ndata[s]) {
#	   if( data_name[[s]][i] %in% data_sd_low) { data_sd[[s]][i] <-
#	                                             data_sd[[s]][i] * mult_sd_low }
#	 }
	 ###########################################################################
     for (i in 1:ndata[s]) {
     ...
     ###########################################################################
     for (i in 1:ndata[s]) {
       if( data_name[[s]][i] == "D_Csoil_f_hay") {
         # data_sd[[s]][i] <- 1000
         data_sd[[s]][i] <- 500
       }
     }
...
## LINKING DATA TO MODEL OUTPUTS
 ...
 # "initialise_CAF2021_general.R", lists the names of all model outputs.
